...
| **Speed** | Not cycle-accurate, and **slow enough that the UI is a slideshow**. Measured 2026-07-30 rather than estimated — the previous "roughly 200x slower" was wrong in both directions. Against the real part's 412 M instructions/second: a realistic synthetic loop (load/store, 4 KB pages, device tick) runs **11.27 M/s**, an **iPhone 6s Plus runs 16.69 M/s** in the app, and a real firmware boot manages **3.82 M/s** — so real guest code is about **108x slower than the hardware**, not 200x, and the phone is *faster* than the x86 development box. Real code costing 3x more than the closest synthetic is not the MMU (the TLB reduces a page walk to +3.5 ns/insn); it is decode and dispatch on branchy, 69%-Thumb code, which is what a translator exists to fix. The guest's clock uses an invented 412 MHz : 6 MHz instruction-to-tick ratio rather than real cycle timing — one chosen constant that the guest can actually observe, which is why it is named here rather than buried. |
