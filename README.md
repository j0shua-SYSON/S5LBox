...
| **Speed** | Not cycle-accurate. An interpreter running roughly **200x slower** than the real 412 MHz part, with an invented 412 MHz : 6 MHz ratio between instructions and the guest's clock rather than real cycle timing. The other exception: one ratio, stated here, and the guest does observe timing derived from it — so this is a place where a chosen constant becomes behaviour the guest can see, which is precisely why it is named rather than buried. |
