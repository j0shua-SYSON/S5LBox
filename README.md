...
| **Storage** | Not flash memory. The desktop harness can create a fresh writable work image per run; the app keeps one persistent work image per machine. New Cydia installations use a 2 GiB HFSX volume. Selecting Jailbreak again on a committed older, smaller installation clones and grows that disk under a separate recovery journal without removing its boot-policy marker. A retained real 500,375,552-byte installed image migrated to exactly 2,147,483,648 bytes with 1,688,907,776 bytes free (78.65%); the install marker, first-boot script, and pinned Cydia archive remained SHA-256-identical. A follow-up real-image allocation test wrote and re-extracted 48 MiB whose three extents all begin beyond the old volume boundary, proving the added blocks are usable rather than header-only. All four durable rename boundaries recover in tests. On a physical iPhone, the exact `96894c9` build first refused the same migration from an ordinary saved session because the mounted, unjournaled HFS volume was correctly marked dirty; after a real guest power-off, both volume headers were clean and the migration succeeded, preserved the install marker, published `guest.storage-v1`, and forced a cold boot. The current tree adds a read-only preflight before any maintenance stage and tells the user to shut down the guest instead of attempting a speculative host-side repair. Cydia then completed its first-run reorganization and respring, but its next launch failed to open `/var/cache/apt/archives/lock` with `EACCES`. Retained-disk inspection found the first concrete cause: this guest predates the first-boot permission fix, so the exact pinned 320,704-byte `Cydia_` executable was root:root `0755`, not setuid/setgid `06755`. A new `guest.cydia-privileges-v1` transaction accepts only that exact binary identity and the exact legacy/desired metadata tuples, changes only its BSD metadata in an unpublished clone, recovers all four rename boundaries, and composes with storage growth. The exact-binary 2 GiB integration passes and independently re-probes the published file as root:root `06755`; physical-device Cydia operation on this build is **not yet tested**. Therefore usable 2 GiB capacity is proven and the observed missing-setuid defect has a tested migration, but **a working Cydia package transaction is still not proven**; a second sandbox or package-manager defect may remain. Back writes one atomic last-session checkpoint and the next open consumes it once. On one physical iPhone 6s Plus running iOS 15.8.5, the saving overlay appeared, exit installed a 122,572,122-byte core state plus the exact 131,248-byte bridge state and a 26-byte last-written marker with no partials, and reopen consumed the marker and restored the same Settings screen at exactly 7,642.4 M instructions. That is one-device lifecycle validation, not broad device coverage. Named Take/Open snapshots remain refused because their copy-on-write lifecycle is unfinished. The canonical imported root filesystem is read-only. |
