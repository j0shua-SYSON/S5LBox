...
| **Storage** | Not flash memory. The desktop harness can create a fresh writable work image per run; the app keeps one persistent work image per machine. Back now writes one atomic last-session checkpoint and the next open consumes it once; the file transaction and core round-trip are host-tested, while the complete phone save/exit/reopen lifecycle is still awaiting device validation. Named Take/Open snapshots remain refused because their copy-on-write lifecycle is unfinished. The canonical imported root filesystem is read-only. |
