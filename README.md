...
- **30 fps is not established.** The currently installed iOS app is reported at
  ...
  and terminal BX/BLX. The restored interval finds 947,961 eligible implemented
  stores (9.480% of fetched instructions) and a 3,372,540-entry store baseline,
  exactly 45,370 entries below the preceding STM baseline. Dense same-binary
  STM and VSTM loops are respectively 4.258x–5.757x and 4.283x–5.266x faster
  with the isolated family on than off on Apple arm64, but neither synthetic
  ratio nor any runner-entry reduction is a frame-rate multiplier. This exact
  app has not been installed and measured
  on a physical phone.
...
| **Speed** | Not cycle-accurate and not yet 30 fps. r458's symmetric live SpringBoard A/B measured **15.694844 M instructions/s** for the desktop app-equivalent core loop versus 14.854045 M/s for its baseline, a repeatable +5.66%. r461/r463 measured execution and publication together at **10.816–11.449 changed scanouts/s on average**, with no window at 30; holding that cadence fixed puts 30 changes/s near **34.3 Minsn/s**, or **2.725x** the metered desktop mean. The current IPA automatically enables a no-JIT, build-time-signed AArch64 graph through the first modeled timebase edge plus terminal single stores, VSTR S/D, transactional one-block ordinary STM and VSTM/VPUSH, and BX/BLX. The extended graph's same-binary geometric-mean gain is **1.483x/1.440x** on macOS 14/15 arm64, but its restored outer-entry opportunity is only **3.573%**. Dense STM and VSTM same-binary loops are respectively **4.258x–5.757x** and **4.283x–5.266x** faster with the isolated family on than off and remain snapshot-exact; real restored reach is 947,961 eligible implemented stores (9.480% fetched), a 3,372,540-entry baseline, and exact incremental VSTR/STM/VSTM reductions of 175,502/124,114/45,370 entries. None of those synthetic ratios or continuity counts is a frame-rate multiplier. The exact app has not been measured on a physical phone; firmware mix, MMU, devices, framebuffer publication and UIKit are absent from the synthetic tests. The installed phone app is still reported at roughly **0–4 fps**, which remains the honest device result. The guest clock still uses an invented 412 MHz : 6 MHz instruction-to-tick ratio rather than cycle timing. |
