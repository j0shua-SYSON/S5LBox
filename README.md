...
| **Storage** | Not flash memory. The desktop harness can create a fresh writable work image per run; the app keeps one persistent work image per machine. Back now writes one atomic last-session checkpoint and the next open consumes it once. On one physical iPhone 6s Plus running iOS 15.8.5, the saving overlay appeared, exit installed a 122,572,122-byte core state plus the exact 131,248-byte bridge state and a 26-byte last-written marker with no partials, and reopen consumed the marker and restored the same Settings screen at exactly 7,642.4 M instructions. That is one-device lifecycle validation, not broad device coverage. Named Take/Open snapshots remain refused because their copy-on-write lifecycle is unfinished. The canonical imported root filesystem is read-only. |
