...
DATA_CACHE_INDEX_BITS = 12
DATA_CACHE_MASK = (1 << DATA_CACHE_INDEX_BITS) - 1
...
def direct_read_body(mnemonic: str, width: int, rd: int) -> list[str]:
    ...
    body.extend([
        # Same page/privilege index as arm_data_cache_try_refill.
        "    ldr w4, [x3, #20]",
        "    lsr w5, w17, #10",
        f"    add w5, w5, w4, lsl #{DATA_CACHE_INDEX_BITS - 1}",
        f"    and w5, w5, #{DATA_CACHE_MASK}",
        "    ldr x6, [x3, #0]",
        "    add x6, x6, w5, uxtw #4",
        "    ldr x16, [x6, #0]",
        "    cbnz x16, 1f",
...
def compact_vfp_memory_body() -> list[str]:
    ...
    return [
        ...
        "    lsr w3, w10, #10",
        f"    add w3, w3, w5, lsl #{DATA_CACHE_INDEX_BITS - 1}",
        f"    and w3, w3, #{DATA_CACHE_MASK}",
        "    add x0, x0, w3, uxtw #4",
        "    ldr x2, [x0, #0]",
        ...
        *compact_tlb_refill_body(False),
        ...
        "    str w4, [x27, #332]",
        "    ldr w3, [x27, #84]",
        "    tbz w15, #1, .La64cr_memory_miss_priv_ready",
        "    mov w3, wzr",
        ".La64cr_memory_miss_priv_ready:",
        "    str w3, [x27, #336]",
        "    ldr w5, [x27, #80]",
        "    str w5, [x27, #340]",
        "    mov w2, #1",
...
def compact_register_memory(prefix: str) -> list[str]:
    ...
    body += [f"{prefix}cached_memory:", f"    cbz x11, {prefix}decode",
             ...
             "    lsr w14, w10, #10",
             f"    add w14, w14, w13, lsl #{DATA_CACHE_INDEX_BITS - 1}",
             f"    and w14, w14, #{DATA_CACHE_MASK}",
             "    add x11, x11, w14, uxtw #4",
...
def compact_raw_function() -> list[str]:
    ...
    return [
        ...
        # Cold PC refusals also inspect that space: mode 3 allows a literal
        # PC base, and an immediate offset's low nibble is not an Rm register.
        # Ordinary DP keeps the same instruction count on its common path.
        ".La64cr_dp_pc_special:",
        "    tbnz w9, #25, .La64cr_fallback",
        ".La64cr_dp_rm_pc_special:",
        "    tbz w9, #4, .La64cr_fallback",
        "    tbz w9, #7, .La64cr_fallback",
        "    b .La64cr_multiply_extra",
        ".La64cr_dp_register_shift:",
        "    tbnz w9, #7, .La64cr_multiply_extra",
        "    ubfx w14, w9, #8, #4",
        "    cmp w14, #15",
        "    b.eq .La64cr_fallback",
        ...
        ".La64cr_dp_sub:",
        "    ldr x14, [x27, #344]",
        "    cbz x14, .La64cr_dp_sub_plain",
        "    mov w15, #0x3001",
        "    movk w15, #0xe042, lsl #16",
        "    cmp w9, w15",
        "    b.ne .La64cr_dp_sub_plain",
        "    mov w0, #1",
        "    b .La64cr_bulk_call",
        ".La64cr_dp_sub_plain:",
        "    sub w10, w11, w10",
        ...
        ".La64cr_dp_mov:",
        "    ldr x14, [x27, #344]",
        "    cbz x14, .La64cr_dp_write",
        "    mov w15, #0xc000",
        "    movk w15, #0xe1a0, lsl #16",
        "    cmp w9, w15",
        "    b.ne .La64cr_dp_write",
        "    mov w0, wzr",
        "    b .La64cr_bulk_call",
        ".La64cr_multiply_extra:",
        # Exact multiply masks exclude synchronization and extra transfers.
        # This cold branch already followed bit4+bit7 rejection in the old
        # decoder; no new test is added to ordinary DP or Thumb instructions.
        "    mov w14, #0xf0",
        "    movk w14, #0x0fc0, lsl #16",
        "    and w15, w9, w14",
        "    cmp w15, #0x90",
        "    b.eq .La64cr_multiply_word",
        "    movk w14, #0x0f80, lsl #16",
        "    and w15, w9, w14",
        "    mov w0, #0x90",
        "    movk w0, #0x0080, lsl #16",
        "    cmp w15, w0",
        "    b.eq .La64cr_multiply_long",
        ".La64cr_bulk_extra_transfer:",
        "    ldr x14, [x27, #344]",
        "    cbz x14, .La64cr_extra_transfer",
        "    mov w15, #0x20d0",
        "    movk w15, #0xe1d0, lsl #16",
        "    cmp w9, w15",
        "    b.ne .La64cr_extra_transfer",
        "    mov w0, #2",
        "    b .La64cr_bulk_call",
        # Preserve every live decode operand/table pointer on refusal. The C
        # helper runs outside the internal VFP RunFast session; a later VFP
        # instruction can reacquire that session through its ordinary gate.
        ".La64cr_bulk_call:",
        "    sub sp, sp, #96",
        "    stp x8, x9, [sp]",
        "    stp x10, x11, [sp, #16]",
        ...
        "    mov w4, w25",
        "    mov w5, w26",
        "    bl A64S_CSYM(a64_compact_raw_bulk_try)",
        "    cbnz w0, .La64cr_bulk_retired",
        "    ldr w0, [sp, #80]",
        "    ldp x8, x9, [sp]",
        "    ldp x10, x11, [sp, #16]",
        "    ldp x12, x13, [sp, #32]",
        ...
        "    b.eq .La64cr_exit",
        "    b .La64cr_loop",
        # MUL/MLA and signed/unsigned long multiply/accumulate. Read every
        # source before writing either destination, including legal aliases.
        # Unpredictable PC operands and duplicate long destinations stay
        # literal. S changes N/Z only, retaining the interpreter's C/V policy.
        ".La64cr_multiply_word:",
        "    ubfx w13, w9, #16, #4",
        "    ubfx w12, w9, #12, #4",
        "    cmp w13, #15",
        "    b.eq .La64cr_fallback",
        ...
        "    cmp w15, #15",
        "    b.eq .La64cr_fallback",
        "    ldr w0, [x19, w14, uxtw #2]",
        "    ldr w1, [x19, w15, uxtw #2]",
        "    mul w0, w0, w1",
        "    tbz w9, #21, .La64cr_multiply_word_value",
        "    ldr w1, [x19, w12, uxtw #2]",
        "    add w0, w0, w1",
        ".La64cr_multiply_word_value:",
        "    str w0, [x19, w13, uxtw #2]",
        "    tbz w9, #20, .La64cr_memory_fallthrough",
        "    tst w0, w0",
        "    b .La64cr_multiply_flags",
        ".La64cr_multiply_long:",
        "    ubfx w13, w9, #16, #4",
        "    ubfx w12, w9, #12, #4",
        "    cmp w13, #15",
        "    b.eq .La64cr_fallback",
        ...
        "    ldr w0, [x19, w14, uxtw #2]",
        "    ldr w1, [x19, w15, uxtw #2]",
        "    tbnz w9, #22, .La64cr_multiply_signed",
        "    umull x0, w0, w1",
        "    b .La64cr_multiply_long_product",
        ".La64cr_multiply_signed:",
        "    smull x0, w0, w1",
        ".La64cr_multiply_long_product:",
        "    tbz w9, #21, .La64cr_multiply_long_value",
        "    ldr w1, [x19, w12, uxtw #2]",
        "    ldr w2, [x19, w13, uxtw #2]",
        "    orr x1, x1, x2, lsl #32",
        "    add x0, x0, x1",
        ".La64cr_multiply_long_value:",
        "    str w0, [x19, w12, uxtw #2]",
        "    lsr x1, x0, #32",
        "    str w1, [x19, w13, uxtw #2]",
        "    tbz w9, #20, .La64cr_memory_fallthrough",
        "    tst x0, x0",
        ".La64cr_multiply_flags:",
        "    mrs x1, nzcv",
        "    ldr w2, [x20]",
        "    lsr w1, w1, #30",
        "    bfi w2, w1, #30, #2",
        "    str w2, [x20]",
        "    b .La64cr_memory_fallthrough",
        # MOV's ordinary path jumps directly to dp_write above. Keep the
        # remaining opcode labels out of the bulk helper's fall-through.
        ".La64cr_dp_bic:",
        "    bic w10, w11, w10",
        "    b .La64cr_dp_write",
        ".La64cr_dp_mvn:",
        ...
        "    b .La64cr_retire",
        "",
        ".La64cr_extra_transfer:",
        # A32 mode 3: LDRH/STRH/LDRSB/LDRSH, immediate/register offsets,
        # either direction and safe pre/post writeback. Doubleword forms and
        # all unaligned halfwords remain literal, including SCTLR.A/U faults.
        # The entire access must pass the shared RAM witness before mutation.
        "    ubfx w14, w9, #5, #2",
        "    cbz w14, .La64cr_fallback",
        "    tbnz w9, #20, .La64cr_extra_kind_ready",
        "    cmp w14, #1",
        "    b.ne .La64cr_fallback",
        ".La64cr_extra_kind_ready:",
        "    ubfx w12, w9, #16, #4",
        "    ubfx w13, w9, #12, #4",
        "    cmp w13, #15",
        "    b.eq .La64cr_fallback",
        "    tbnz w9, #24, .La64cr_extra_pre_guard",
        "    tbnz w9, #21, .La64cr_fallback",
        "    b .La64cr_extra_writeback_guard",
        ".La64cr_extra_pre_guard:",
        "    tbz w9, #21, .La64cr_extra_offset",
        ".La64cr_extra_writeback_guard:",
        "    cmp w12, #15",
        "    b.eq .La64cr_fallback",
        "    cmp w12, w13",
        "    b.eq .La64cr_fallback",
        ".La64cr_extra_offset:",
        "    tbz w9, #22, .La64cr_extra_register_offset",
        "    ubfx w14, w9, #8, #4",
        "    and w0, w9, #0xf",
        "    orr w14, w0, w14, lsl #4",
        "    b .La64cr_extra_base",
        ".La64cr_extra_register_offset:",
        "    tst w9, #0xf00",
        "    b.ne .La64cr_fallback",
        "    and w0, w9, #0xf",
        "    cmp w0, #15",
        "    b.eq .La64cr_fallback",
        "    ldr w14, [x19, w0, uxtw #2]",
        ".La64cr_extra_base:",
        "    cmp w12, #15",
        "    b.ne .La64cr_extra_base_register",
        "    add w8, w26, #8",
        "    b .La64cr_extra_update",
        ".La64cr_extra_base_register:",
        "    ldr w8, [x19, w12, uxtw #2]",
        ".La64cr_extra_update:",
        "    tbz w9, #23, .La64cr_extra_sub",
        "    add w11, w8, w14",
        "    b .La64cr_extra_updated",
        ".La64cr_extra_sub:",
        "    sub w11, w8, w14",
        ".La64cr_extra_updated:",
        "    mov w10, w8",
        "    tbz w9, #24, .La64cr_extra_address",
        "    mov w10, w11",
        ".La64cr_extra_address:",
        # Only SH=2 (LDRSB) may access an odd address in this native tier.
        "    ubfx w14, w9, #5, #2",
        "    cmp w14, #2",
        "    b.eq .La64cr_extra_lookup",
        ...
        "    mov x6, x0",
        "    mov x7, x1",
        "    tbnz w9, #20, .La64cr_extra_load",
        "    ldr w0, [x19, w13, uxtw #2]",
        "    strh w0, [x6]",
        "    b .La64cr_memory_commit",
        ".La64cr_extra_load:",
        "    tbz w9, #5, .La64cr_extra_signed_byte",
        "    tbnz w9, #6, .La64cr_extra_signed_half",
        "    ldrh w0, [x6]",
        "    b .La64cr_memory_load_register",
        ".La64cr_extra_signed_half:",
        "    ldrsh w0, [x6]",
        "    b .La64cr_memory_load_register",
        ".La64cr_extra_signed_byte:",
        "    ldrsb w0, [x6]",
        "    b .La64cr_memory_load_register",
        "",
        ...
        *compact_vfp_nonarith_body(),
        ...
        "    ldr x14, [x27, #344]",
        "    cbz x14, .La64cr_thumb_high_mov_plain",
        "    sub w15, w26, w23",
        "    add w15, w15, #4",
        "    cmp w15, w24",
        "    b.hi .La64cr_thumb_high_mov_plain",
        "    sub w15, w15, #2",
        "    ldrh w15, [x22, w15, uxtw]",
        "    and w15, w15, #0xfe00",
        "    mov w14, #0x5800",
        "    cmp w15, w14",
        "    b.ne .La64cr_thumb_high_mov_plain",
        "    mov w0, #3",
        "    b .La64cr_bulk_call",
        ".La64cr_thumb_high_mov_plain:",
        "    mov w14, #13",
        "    b .La64cr_dp_plain_dispatch",
        "",
        ...
        "    ldr x14, [x27, #344]",
        "    cbz x14, .La64cr_thumb_memory_reg_plain",
        "    mov w15, #0x5919",
        "    cmp w9, w15",
        "    b.ne .La64cr_thumb_memory_reg_plain",
        "    mov w0, #4",
        "    b .La64cr_bulk_call",
        ".La64cr_thumb_memory_reg_plain:",
        "    and w13, w9, #7",
        "    ubfx w10, w9, #3, #3",
        "    ldr w10, [x19, w10, uxtw #2]",
        "    ubfx w11, w9, #6, #3",
        ...
        "    cbz x0, .La64cr_thumb_memory_cache_fail",
        "    cbz x1, .La64cr_thumb_memory_cache_fail",
        "    ldr w2, [x27, #84]",
        "    lsr w3, w10, #10",
        f"    add w3, w3, w2, lsl #{DATA_CACHE_INDEX_BITS - 1}",
        f"    and w3, w3, #{DATA_CACHE_MASK}",
        "    add x0, x0, w3, uxtw #4",
        "    ldr x2, [x0, #0]",
        "    cbz x2, .La64cr_thumb_memory_cache_fail",
        "    lsr w3, w10, #10",
        "    lsl w3, w3, #10",
        "    ldr w4, [x27, #84]",
        "    orr w3, w3, w4",
        "    ldr w4, [x0, #8]",
        "    cmp w4, w3",
        "    b.ne .La64cr_thumb_memory_cache_fail",
        "    ldr w4, [x0, #12]",
        "    ldr w3, [x27, #80]",
        "    cmp w4, w3",
        "    b.ne .La64cr_thumb_memory_cache_fail",
        ...
        *compact_tlb_refill_body(True),
        ...
        "    blr x9",
        "    mov w11, w0",
        # The callback has consumed this one-shot witness. A retry may publish
        # a new miss, but an unrelated later fallback must never see it.
        "    str wzr, [x27, #320]",
        "    ldr w26, [x19, #60]",
        "    cbz w11, .La64cr_exit",
        "    cmp w11, #3",
