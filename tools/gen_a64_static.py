...
def compact_raw_function() -> list[str]:
    ...
    return [
        ...
        # x28 keeps CPSR resident until a C boundary and w7 is the live width.
        "    stp x29, x30, [sp, #-112]!",
        "    stp x19, x20, [sp, #16]",
        "    stp x21, x22, [sp, #32]",
        "    stp x23, x24, [sp, #48]",
        "    stp x25, x26, [sp, #64]",
        "    stp x27, x28, [sp, #80]",
        "    str w6, [sp, #96]", # original budget
        "    mov x19, x0",       # architectural register file
        "    mov x20, x1",       # caller-owned CPSR word
        ...
        "    ldr w28, [x20]",    # resident architectural CPSR
        "    mov w7, #4",        # current A32/Thumb instruction width
        ...
        # Select width from resident CPSR.T. Native interworking updates w28;
        # an exact fallback is reloaded before execution resumes.
        "    tbnz w28, #5, .La64cr_thumb_fetch",
        "    mov w7, #4",
        "    tst w8, #3",
        "    b.ne .La64cr_exit",
        "    add w11, w8, #4",
        ...
        *compact_vfp_nonarith_body(),
        ...
        # w7 remains two, so normal fallthrough still retires one Thumb
