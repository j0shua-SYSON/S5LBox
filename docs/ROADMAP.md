...
| **M5** | Userspace → SpringBoard | `launchd` runs; the home screen renders and takes a tap | 🔵 **in progress; the multi-run telephony blocker is resolved.** Un-matching the unmodelled baseband nubs let CommCenter call `bootstrap_check_in` successfully (`r0=0`, SUCCESS arm), so SpringBoard's telephony singleton now enters *and returns* instead of blocking forever. The boot is past `applicationDidFinishLaunching:` and `isTethered`, with CLCD descriptor refreshes up 1→7→27. Since then it has cleared `FPSCR.RMode` (implemented, not refused) and the ARMv6 parallel add/subtract family, reaching 2,191,848,855 instructions. `UIController` is still unreached and the framebuffer is still the seed, so **not rendered**. Historical detail below. |
