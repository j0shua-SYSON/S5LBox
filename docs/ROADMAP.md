...
| **M5** | Userspace → SpringBoard | `launchd` runs; the home screen renders and takes a tap | 🔵 **in progress — it renders; it cannot yet be tapped, and the screen it draws is the activation screen.** Two blockers were cleared on 2026-07-26. First, the SpringBoard crash loop: 35 byte-identical `ReportCrash` reports at `_mbx2DDisable+0x20` because un-matching `/arm-io/mbx` left `enable_mbx2d` set with a NULL context. `CA_ENABLE_MBX2D=0` in SpringBoard's launchd environment fixed it — verified at instruction level *and now run*: 30 respawns became 1, and SpringBoard built its UI controller, created its window and called `orderFront` for the first time. Second, and the reason nothing had ever drawn: `/device-tree/vram` ships `reg = {0,0}` and iBoot fills it, so `IOSurfaceRoot` could not publish `PurpleGfxMem`, `AppleH1CLCD` fell back to `withPhysicalAddress(..., kIODirectionOut)`, and `IOSurfaceClient` maps any output-only descriptor `kIOMapReadOnly` — userspace received the framebuffer **read-only** and the compositor faulted on its first store. One `dt_set_reg` call (`691b727`) produced run59: **14,264,987 changed scanout bytes**, 97,510 of 460,800 framebuffer bytes non-zero against 384 in every prior run, and a real frame at `docs/images/run59-first-frame.png`. run54 is the negative control — 22e9 instructions and 33 SpringBoard launches *without* the fix produced zero changed bytes, so this was never a matter of runtime. **Both of those blockers are now cleared, and M5 is still not met** — which is the useful part. Activation is provisioned: the HFS+ catalog writer landed in `33ce6b5`, `-ActivationState = FactoryActivated` plus `-BrickState = false` are written into `/private/var/root/Library/Lockdown/data_ark.plist` in the work image, and it works — the iTunes-connect artwork is gone. What replaced it is the boot spinner, **not** a home screen. run66 gave it 12 billion instructions: `CATransaction-flush` **0** hits, last scanout write at instruction 1,887,035,649, final frame 1,833 of 460,800 non-zero bytes, and the `lockdownd` RSA keygen — the standing "it is just slow" explanation — ran to completion between 8.0e9 and 8.5e9 without changing anything. Touch is built to the driver: `core/src/soc/mtz2.c` and `gpioic.c` landed, and run77 delivered four reports that Apple's `AppleMultitouchZ2SPI` read and whose payload checksums it accepted (probe `0xc04413e8`, `r0 == r2` on all four); run90 then took a contact all the way to `__UIApplicationHandleEvent`. **The reason a tap changed nothing is now measured rather than guessed, and it was none of the things previously suspected.** Every touch normalised to the same point because the surface bounds were *negative* — read directly out of run96's snapshot at `grid+0x148`: `Xmax -434, Xmin -75, Ymax -439, Ymin -75`. Those come from the Sensor Region Descriptor's electrode counts, which come from the `Sensor Rows`/`Sensor Columns` IORegistry properties, and **every int32 property was missing**: the guest held MultitouchSupport's own fallbacks (5000/7500) rather than this device's 4800/7200. run100's probes, with positive controls, found why — the driver never issued a **single control read** in two billion instructions. A Z2 has no flash and is bootloaded on every boot; the model claimed to be an unprogrammed part and then refused to be programmed, so it never ran application firmware and the driver correctly declined to interrogate it. The HBPP bootloader is now reversed and implemented (`docs/multitouch.md` §6) and the one-time `hbpp_answered` claim is gone. **First boot with it is in flight; no tap has yet been shown to land.** **UPDATE 2026-07-29 — the cause is found and it is two octets.** The bootload transfers the whole 54,156-octet image and never completes (`exec 0`), and the driver stalling on it is ALSO what stopped the display: the boot halts after `systemShutdown false`, nothing composites, and the CLCD frame interrupt is never enabled. run140 measured that un-matching the digitizer restores the lock screen (273,206 non-zero bytes against 1,821), so `--multitouch` now exists and defaults OFF as a temporary trade. run144 dumped the stream and found the image is ONE well-formed HBPP DATA packet — `30 01 34 df 00 00 00 00 01 13`, header checksum verifying exactly at 0x113 and 13,535 words matching the file size — preceded by two octets `18 E1` that this model did not know. The framer ate the `0x18` as unknown and read the `0xE1` as GET_CMD_STATUS, whose sixteen octets swallowed the DATA header; after that it parsed ARM code as commands, ARM's `AL` nibble being `0xe` against command opcodes `0xe1`..`0xee`. `MTZ2_OP_HBPP_IDLE` is implemented; whether correct framing is SUFFICIENT is under measurement in run146. See `docs/multitouch.md` §6.9. What actually remains is one thing, and it is now located rather than guessed: SpringBoard opens the touchscreen's IOKit user client — proven from the MIG reply bytes, `io_service_open_extended(0xb503) -> 0xb603`, with `"Sensor Surface Width"` and nine sibling property names identifying the service — and blocks in the fourth `io_connect_method` on it, at `assert_wait(0xc0d8a358, THREAD_UNINT)`. Whether a delivered frame releases it is under measurement. Full specification, verified addresses and the remaining build order are in handoff **§23.4**; the six retractions from the earlier reading of this milestone are in **§13.0j** and **§23.8**. **UPDATE 2026-07-30 — THE BOOTLOAD COMPLETES AND THE DRIVER READS A TOUCH.** run163: `exec 1`, `in-hbpp 0`, all four calibration register writes landed (`10001c04`, `10001c08`, `1000300c`, `1000304c`), the `1f:2` calibration request fired and the `1d:12` execute packet arrived — every stage of §6.4's state machine, in order, once, with `unknown-opcodes 0`. The part stopped being a bootloader. run164 then sent a real single-contact drag and the guest CONSUMED it: `queued 26 / length-reads 26 / data-reads 26 / read 26 / refused 0`. **And the digitizer no longer costs the screen** — run140 (un-matched) and run163/164 (matched) both render 273,206 non-zero bytes, so the trade `--multitouch` was created to make is no longer being charged. Four fixes got there, none of them reasoned out: (1) a chip-select edge split the DATA header in half and the framer discarded it (`65e704e`); (2) the driver was blocked in the ATN_ACK, not the DMA send — my own elimination argument picked the wrong one of two calls, and call probes refuted it (run155); (3) a TX-only DMA burst left the receive FIFO full so the next PIO transfer could never shift, `tx/rx level 2/8` for 2.44 G instructions (`a6f3520`); (4) the register-write helper at `0xc0440e4c` compares its acknowledgement against **0x4AD1**, not the DATA sender's 0x4BC1 — two senders, two literals, and §6.2 documents only one. **Still not met, and the gap is precise:** no gesture has been shown to CHANGE the screen. The reports reach the driver; whether they reach `__UIApplicationHandleEvent` is what run167 measures, after run165 showed the guest is still launching MobileMail at 3.55 G and run164's drag at 4.0 G may simply have arrived before the UI could take it. See BOOTLOG 2026-07-30 and multitouch.md §6.13–§6.16. **UPDATE 2026-07-30 (later) -- THE FUNNEL IS OPEN.** run170 probed every stage run90 named, with a correct 24-step `--drag` and a programmed digitizer: kernel enqueue **28**, crossed to userspace **27**, `_mt_FillMTContactDirectFromBinary` **26**, MultitouchHID **26**, `__UIApplicationHandleEvent` **31** -- against run90's 16/17/-/9/**2**. `queued 26 / read 26 / refused 0` in, 26 contacts at UIKit. **Nothing is dropped at any layer; touch is delivered end to end.** The screen is still 273,206 bytes unchanged, so what remains is not plumbing but WHERE the touch lands -- and the named suspect is run96's negative surface bounds (`Xmax -434` for `4656`), which were negative because the driver never interrogated an unprogrammed part. run171 reads whether it interrogates now. M5 asks for a home screen that TAKES A TAP; the tap now arrives, and the coordinate transform is the last piece. **UPDATE 2026-07-30 (final) -- THE PHONE UNLOCKS.** r181 and r182 both completed a slide-to-unlock and reached iPhone OS 3's home screen -- status bar, the first-run *Edit Home Screen* tip, and the dock (Phone, Mail, Safari, iPod): `docs/images/r181-unlocked-home-screen.png`. 80.756% of pixels differ from the lock screen. **The only defect was the gesture's length**: every earlier drag stopped at x=265, the knob lags the contact by ~8 px, so it fell short of the threshold and iOS sprang it home. Driving to x=305 crosses it. A host-delivered gesture now drives a real UIKit control to completion, which is item 4 of the milestone list below. **UPDATE 2026-07-30 (later still) -- THE SLIDER WAS TRACKING ALL ALONG, AND THE COORDINATE THEORY WAS WRONG.** The unlock knob follows the finger: knob centre 57.5 (after the lift) -> **80.5** at 4.03 G -> **156.5** at 4.09 G, same 67-pixel width translated linearly with the contact, 4,245 changed pixels all inside x 22..272 / y 410..456. Everything the coordinate theory predicted would be broken was measured CORRECT -- bounds `-75/4656/-75/7275` exact in the running guest, clamped X 745 and Y 837 exact, report spacing 16.000 ms across all 25 intervals, Z amplitude 160 intact, MultitouchHID's touch flag set. **The failure was in the instrument, not the emulator:** every screenshot was captured at 5.5 G, ~4 guest-seconds after the finger lifted at 4.165 G, and iOS springs the knob home when a slide does not complete -- so a working drag and no drag render the same frame. Five runs were spent eliminating hypotheses about a failure that was not occurring; run170's own 26-of-26 contacts at `__UIApplicationHandleEvent` had already said the path worked. Retracted: that the knob never moved, that contacts landed elsewhere, and that a mirrored drag could separate the two Y conventions. **What is established is TRACKING of one contact.** Whether a slide crosses the unlock threshold, and whether two simultaneous contacts work, are separate measurements -- the multi-contact path is built and unit-tested but no run has yet put two fingers down. Method now standing: for anything animated, capture DURING the event, and treat "identical to baseline" as a null result until a control that should differ proves the instrument can see a change. Affordable only because of the 3.9 G checkpoint -- 4.4 min per experiment against ~76 for a cold boot, validated exactly against a cold boot's registers. See BOOTLOG 2026-07-30 r179/r180 and multitouch.md §6.17-§6.18. |
...
| **D** | Dynarec (parallel) | SpringBoard at interactive frame rates on the phone | 🔵 emitter + ARM/Thumb translator and host execution tests exist (off by default); no code cache or dispatcher calls them. **2026-07-30 (revised): the target is 30 fps ON THE INTERPRETER ALONE. The "60 fps with JIT" half of the goal is WITHDRAWN.** S5LBox has to run on stock iPhones, and a JIT there needs the debugger-attach workaround (StikDebug / JitStreamer) or a jailbreak; a build most people cannot install is not the product. The JIT stays in tree as future work for jailbroken devices and may still be offered behind a flag, but it is never load-bearing for the default experience and no frame-rate plan may depend on it. Interpreter ceiling is 15-30 M insn/s against 250-370 "needed for realtime" — **that comparison is wrong and drove this row for weeks.** Frame rate is `host_rate / instructions_per_frame`, and guest idle is free because WFI fast-forwards (`_machine_idle` is entered 23,799 times in r195 and does not appear in the profile at all). Executing a guest second per host second was never the requirement; only per-frame work is.  banked this session is ~8% (fetch-block cache, TLB lookups 1.10 -> 0.12/insn) plus a TLB index de-alias that cut misses 11,096,237 -> 130,477. The lever is removing per-frame work, not executing instructions faster: the guest software-composites every pixel because we un-match the GPU, which is work the real iPhone never did — the MBX exists precisely to avoid it, so 60 fps on the software path means beating the original device at a task it declined. **Graphics HLE needs no JIT** (native code already signed, interception is a PC compare, nothing generated at runtime) and is the only single change that could reach 30 fps; 60 fps would need HLE *and* the interpreter work *and* the JIT, which is precisely why 60 is no longer a target. Modelling MBX2D is dominated by CoreGraphics leaf HLE — undocumented command stream, and the host rasteriser has to be written either way. **The load-bearing unknown is instructions-per-frame, which nothing had ever measured**; `CLCD_UPDATE == 2` is now counted and reported as a ring of gaps (not a mean — a resting screen and a full-screen transition differ by orders of magnitude). run168 is taking that measurement. Note also that this row's own §1.1/§1.2 figures were stale enough to mislead two independent research passes; see the corrections there. **2026-07-30, what the profile actually says:** the 65.3% `[userspace]` bucket was opaque only because `diagnostic_pc_name()` prints every user PC under that one literal. The addresses were always in the log. All sixteen hottest PCs of r187, r194 and r195 alike fall in one 88-byte span, 0x3145ad4c..0x3145ada4, which `tools/dscmap.py` resolves to `_mulg_common` in Security.framework, neighboured by `_SECOID_FindOID`, `_DEREncodeSequence`, `_modg_via_recipSmall` and `_ginverseMod` — CryptKit giant-number arithmetic beside ASN.1 DER and OID lookup. With `_SHA1Init` at 17.8% of the same profile, the largest single expense is verifying Apple's code signatures, not drawing. Those are leaf, pure, exactly-specified functions: the easiest HLE targets in the tree, and they need no JIT. Whole-run share is ~30-35% (Amdahl 1.43-1.54x); the FRAME-WINDOW share is not yet measured and the two are different questions. Also refuted today: decode scatter costs nothing (a ten-position mixed loop measures 14.45 M insn/s against the single-position load/store loop's 14.18 in the same interleaved sweep), so the table-dispatch rewrite the ARM/Thumb gap seemed to justify is not worth doing. Still standing and unbuilt: the device tick at 16.8%. |
| **N** | Guest networking (parallel) | The guest resolves a name and fetches a URL | 🔵 **S0 met 2026-07-27 — the guest transmits.** run80: `Connect: ppp0 <--> /dev/tty.debug`, then 47 bytes on uart4 that are one complete LCP Configure-Request — `7E FF 7D 23 C0 21 …`, RFC 1662 framing, the four options `pppd` 2.4.2 asks for by default. `pppd` did not exit; it is waiting for a reply. **Nothing answers**, so no address is negotiated and no packet has been carried: this is one direction of one link layer, not networking. The emulator did not change to achieve it — one argument did, `/dev/uart.debug` → `/dev/tty.debug`, because the former is a real character device that is not a tty (`d_ttys = NULL`, `d_type = 0`, `TIOCSETD` falling to ENOTTY at `0xc046fe30`, and `ttioctl` unreachable from that path). **The host endpoint now exists.** run98 brought the link up — LCP Opened, IPCP Opened, `local IP address 10.0.2.15`, `remote IP address 10.0.2.2` — and 2026-07-28 completed N0 above it: `core/src/net/net.c` and `tcp.c` are an IPv4 stack, NAT and resolver with no socket, no clock and no platform header in them, `tools/net_host.c` is the only thing that touches a socket and needs nothing privileged, and `bootkernel --nat` wires the two to the PPP peer. Its own suite found three defects that a guest could never have diagnosed — most of all that **no TCP connection could ever have reached ESTABLISHED**, because the acceptable-ACK ceiling was computed from the send buffer and a SYN is not buffer content. 230 assertions across the two suites. **Not yet demonstrated end to end: no run has carried a guest datagram through it**, so this is a complete and tested host half, not a fetched URL. |
| **A** | Guest audio (first-device track) | Guest PCM reaches the host speaker without blocking the CPU thread | ⚪ priority, not designed or built — but one blocker was removed on 2026-07-28 before it was ever hit. I2S is a DMA destination whose FIFOs the device tree programs at SIXTEEN bits, and `machine.c` decoded that window with `mmio_word()`, which takes 32-bit accesses only. Every narrow DMA store into it would have fallen through to the unmapped path, exactly as was measured happening to the touch controller's firmware download (run114: 812,340 bytes moved by the controller, 176 words shifted by the port, 813,135 unmapped writes). `mmio_data()` fixes both ports at once; `test_pl080.c` had predicted this one in a comment — "the next thing to fix if the FIFO is ever modelled" |
