...
| **M5** | Userspace → SpringBoard | `launchd` runs; the home screen renders and takes a tap | 🔵 **in progress; the blocker is a SpringBoard crash loop, and nothing has rendered.** SpringBoard is killed and respawned by `launchd` roughly every 470 M instructions: the guest's own `ReportCrash` wrote **35** byte-identical reports — `EXC_BAD_ACCESS (SIGBUS)`, `KERN_PROTECTION_FAILURE at 0x00000048`, crashed thread 3, `pc 0x30e1ea50` in `_mbx2DDisable+0x20`, reached from `CA::RenderMBX2D::render` on the IOMFB vblank callback. Un-matching `/arm-io/mbx` means no `AppleMBXDevice` is published, so `_mbx2DCtxInitialize` fails and `_mbx2DGlobalContext` (`0x381200d8`) stays NULL, but `MBXServer::MBXServer` never clears `enable_mbx2d` — the first vblank stores through NULL. **The fix requires no GPU emulation:** `CA_ENABLE_MBX2D=0` in SpringBoard's launchd environment makes `mbx2d_context()` (`0x31241a8c`) return NULL early and drops `render_update` into QuartzCore's shipped CPU compositor (`CARenderOGLNew(_kCARenderSoftwareCallbacks)` → `CA::OGL::SWContext`). That is **verified at instruction level from disassembly and has not been run.** Earlier readings of this milestone as a budget, latency, or checkpoint-fidelity problem are retracted; restore is bit-exact (27/27 identical heartbeat PC samples) and the apparent divergence was a trace block reporting only the newest SpringBoard generation. Historically, and still true as far as it goes: the multi-run telephony blocker is resolved: un-matching the unmodelled baseband nubs let CommCenter call `bootstrap_check_in` successfully (`r0=0`, SUCCESS arm), and SpringBoard's telephony singleton now *completes*, `CTCenterGetDefault` call and return both recorded. Run35 then reached `SpringBoard:UIController-call`, hits=1 at 3,478,858,148 — the first time ever. Runs 30–33 saw 0 hits only because `UIController` lies past their 2.5e9 cap, so a clean cap-stop carries **no information** about blockers. Three CPU gaps were cleared as whole families on the way (`FPSCR.RMode` implemented rather than refused; the ARMv6 parallel add/subtract family). The frontier is not a CPU gap: 99.6% of the final window is userspace, ~40% of it in `Security.framework` `_mulg_common`, a 16-bit-limb giant-integer multiply that is bounded arithmetic rather than a wait — an accurate account of where time goes, but **not** the reason nothing renders. CLCD is live and correct (`scanning=1`, `frames=1026`, 320×480, refreshes 1→7→27→71→102) and the framebuffer is still the seed — identical in every run since run35, because nothing was ever going to draw — so **not rendered and no tap**. Next step: run SpringBoard with `CA_ENABLE_MBX2D=0`. Detail in `docs/BOOTLOG.md` (2026-07-26) and handoff **§13.0j**, which also lists the six retractions. |
