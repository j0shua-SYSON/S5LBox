...
/* VLD1/VST1 multiple single elements, A1/T1 (DDI0406C.b A8.8.320/404).
 * Lane/replicate and interleaved structure families have separate encodings.
 * Keep unsupported element sizes in the checked space so EN=0 cannot turn
 * a capability stop into a guest lazy-enable fault. */
static bool a8_neon_single_elements_space(const arm_cpu_t *c, uint32_t insn) {
...
/* VLD1 single element to one lane (A8.8.321). Only naturally aligned
 * .32 loads are implemented; other sizes and replicate forms stay checked. */
static bool a8_neon_lane_load_space(const arm_cpu_t *c, uint32_t insn) {
    uint32_t prefix = (c->cpsr & ARM_CPSR_T) ? 0xf9a00000u : 0xf4a00000u;
    return c->arch == ARM_ARCH_V7_CORTEX_A8 && (insn & 0xffb00300u) == prefix;
}

static arm_status_t exec_a8_neon_lane_load(arm_cpu_t *c, uint32_t insn) {
    unsigned size = (insn >> 10) & 3u, index_align = (insn >> 4) & 15u;
    unsigned d = ((insn >> 12) & 15u) | ((insn >> 18) & 16u);
    unsigned rn = (insn >> 16) & 15u, rm = insn & 15u;
    if (size != 2u || rn == 15u || ((index_align & 7u) != 0u && (index_align & 7u) != 3u))
    ...
    if (address & 3u) {
        if ((index_align & 3u) || (c->cp15.sctlr & ARM_SCTLR_A)) {
    ...
    unsigned shift = (index_align >> 3) * 32u;
    uint64_t mask = UINT64_C(0xffffffff) << shift;
    vfp_set_d(c, d, (vfp_get_d(c, d) & ~mask) | (uint64_t)value << shift);
    /* B1.9.8 restores the base on synchronous aborts. Delay both results
     * until the single element completed, also preserving checked retries. */
...
static arm_status_t exec_a8_neon_single_elements(arm_cpu_t *c, uint32_t insn) {
    ...
    unsigned rn = (insn >> 16) & 15u, rm = insn & 15u;
    bool load = (insn & (1u << 21)) != 0u;
    if (size < 2u || rn == 15u || first + count > 32u ||
        (count != 4u && (count == 2u ? align == 3u : align >= 2u))) return ARM_UNDEFINED;
    if (!vfp_cpacr_permits(c) || !vfp_enabled(c)) return ARM_GUEST_UNDEFINED;
    if (c->cpsr & ARM_CPSR_E) return ARM_UNDEFINED;
     ...
     * permits retry after a checked host-bus failure, retaining the prefix. */
    if (rm != 15u) c->r[rn] = updated;
    return ARM_OK;
}
...
/* VEXT A1/T1 (DDI0406C.b A8.8.316). The four-bit immediate is measured
 * in bytes; .16/.32/.64 spellings are assembler aliases, not size fields. */
static bool a8_neon_extract_space(const arm_cpu_t *c, uint32_t insn) {
    uint32_t prefix = (c->cpsr & ARM_CPSR_T) ? 0xefb00000u : 0xf2b00000u;
    return c->arch == ARM_ARCH_V7_CORTEX_A8 && (insn & 0xffb00010u) == prefix;
}

static arm_status_t exec_a8_neon_extract(arm_cpu_t *c, uint32_t insn) {
    unsigned d = ((insn >> 12) & 15u) | ((insn >> 18) & 16u);
    unsigned n = ((insn >> 16) & 15u) | ((insn >> 3) & 16u);
    unsigned m = (insn & 15u) | ((insn >> 1) & 16u), quad = (insn >> 6) & 1u;
    unsigned offset = (insn >> 8) & 15u;
    if ((quad && ((d | n | m) & 1u)) || (!quad && offset >= 8u)) return ARM_UNDEFINED;
    if (!vfp_cpacr_permits(c) || !vfp_enabled(c)) return ARM_GUEST_UNDEFINED;
    uint64_t source[4], result[2];
    for (unsigned r = 0; r <= quad; r++) {
        source[r] = vfp_get_d(c, n + r);
        source[1u + quad + r] = vfp_get_d(c, m + r);
    }
    unsigned word = offset / 8u, shift = (offset % 8u) * 8u;
    for (unsigned r = 0; r <= quad; r++) {
        result[r] = source[word + r] >> shift;
        if (shift) result[r] |= source[word + r + 1u] << (64u - shift);
    }
    /* Read both full operands before an aliased write. A zero shift is a
     * word copy, avoiding C's undefined shift by64. All flags are untouched. */
    for (unsigned r = 0; r <= quad; r++) vfp_set_d(c, d + r, result[r]);
    return ARM_OK;
}
...
static bool a8_neon_immediate_space(const arm_cpu_t *c, uint32_t insn) {
    if (c->arch != ARM_ARCH_V7_CORTEX_A8) return false;
    return (c->cpsr & ARM_CPSR_T) ? (insn & 0xefb80090u) == 0xef800010u
                                : (insn & 0xfeb80090u) == 0xf2800010u;
}

static arm_status_t exec_a8_neon_immediate(arm_cpu_t *c, uint32_t insn) {
    unsigned d = ((insn >> 12) & 15u) | ((insn >> 18) & 16u), quad = (insn >> 6) & 1u;
    unsigned mode = (insn >> 8) & 15u, op = (insn >> 5) & 1u;
    uint32_t imm = (insn & 15u) | ((insn >> 12) & 0x70u) |
        ((insn >> ((c->cpsr & ARM_CPSR_T) ? 21u : 17u)) & 0x80u);
    if ((quad && (d & 1u)) || (op && mode == 15u) ||
        (!imm && ((mode >= 2u && mode <= 7u) || (mode >= 10u && mode <= 13u)))) return ARM_UNDEFINED;
    if (!vfp_cpacr_permits(c) || !vfp_enabled(c)) return ARM_GUEST_UNDEFINED;

    uint64_t value;
    if (mode < 8u) {
        uint32_t word = imm << ((mode >> 1) * 8u);
        value = (uint64_t)word << 32 | word;
    } else if (mode < 12u) {
        uint32_t half = imm << (((mode >> 1) & 1u) * 8u);
        value = half * UINT64_C(0x0001000100010001);
    } else if (mode < 14u) {
        uint32_t word = mode == 12u ? (imm << 8) | 0xffu : (imm << 16) | 0xffffu;
        value = (uint64_t)word << 32 | word;
    } else if (mode == 14u) {
        if (!op) value = imm * UINT64_C(0x0101010101010101);
        else {
            value = 0;
            for (unsigned b = 0; b < 8u; b++) if (imm & (1u << b)) value |= UINT64_C(0xff) << (b * 8u);
        }
    } else {
        uint32_t word = ((imm & 0x80u) << 24) | ((imm & 0x40u) ? 0x3e000000u : 0x40000000u) |
            ((imm & 0x3fu) << 19);
        value = (uint64_t)word << 32 | word;
    }
    /* Odd cmodes below 12 update the destination; other forms replace it.
     * op=1,cmode=14 expands individual bits to bytes for VMOV.I64, not VMVN. */
    bool update = mode < 12u && (mode & 1u);
    if (op && mode != 14u) value = ~value;
    for (unsigned r = 0; r <= quad; r++) {
        uint64_t result = update ? (op ? vfp_get_d(c, d + r) & value : vfp_get_d(c, d + r) | value) : value;
        vfp_set_d(c, d + r, result);
    }
    return ARM_OK;
}

/* A8.8.305: include reserved sizes in the checked allocation so encoding
 * validity precedes availability. Only size=2 (32-bit lanes) is defined. */
static bool a8_neon_integer_space(const arm_cpu_t *c, uint32_t insn) {
    uint32_t prefix = (c->cpsr & ARM_CPSR_T) ? 0xffb30600u : 0xf3b30600u;
    return c->arch == ARM_ARCH_V7_CORTEX_A8 && (insn & 0xffb30e10u) == prefix;
}

static arm_status_t exec_a8_neon_integer(arm_cpu_t *c, uint32_t insn) {
    unsigned d = ((insn >> 12) & 15u) | ((insn >> 18) & 16u);
    unsigned m = (insn & 15u) | ((insn >> 1) & 16u), quad = (insn >> 6) & 1u;
    unsigned op = (insn >> 7) & 3u;
    if (((insn >> 18) & 3u) != 2u || (quad && ((d | m) & 1u))) return ARM_UNDEFINED;
    if (!vfp_cpacr_permits(c) || !vfp_enabled(c)) return ARM_GUEST_UNDEFINED;
    uint64_t result[2]; uint32_t exceptions = 0u;
    for (unsigned r = 0; r <= quad; r++) {
        uint64_t value = vfp_get_d(c, m + r);
        uint32_t lo = vfp_a8_neon_integer((uint32_t)value, op >> 1, !(op & 1u), &exceptions);
        uint32_t hi = vfp_a8_neon_integer((uint32_t)(value >> 32), op >> 1, !(op & 1u), &exceptions);
...
/* VMAX/VMIN.F32 A1/T1 (DDI0406C.b A8.8.335), including reserved sz. */
static bool a8_neon_minmax_space(const arm_cpu_t *c, uint32_t insn) {
    uint32_t prefix = (c->cpsr & ARM_CPSR_T) ? 0xef000f00u : 0xf2000f00u;
    return c->arch == ARM_ARCH_V7_CORTEX_A8 && (insn & 0xff800f10u) == prefix;
}

static arm_status_t exec_a8_neon_minmax(arm_cpu_t *c, uint32_t insn) {
    unsigned d = ((insn >> 12) & 15u) | ((insn >> 18) & 16u);
    unsigned n = ((insn >> 16) & 15u) | ((insn >> 3) & 16u);
    unsigned m = (insn & 15u) | ((insn >> 1) & 16u), quad = (insn >> 6) & 1u;
    if ((insn & (1u << 20)) || (quad && ((d | n | m) & 1u))) return ARM_UNDEFINED;
    if (!vfp_cpacr_permits(c) || !vfp_enabled(c)) return ARM_GUEST_UNDEFINED;
    bool minimum = (insn & (1u << 21)) != 0u;
    uint64_t result[2]; uint32_t exceptions = 0u;
    for (unsigned r = 0; r <= quad; r++) {
        uint64_t a = vfp_get_d(c, n + r), b = vfp_get_d(c, m + r);
        uint32_t lo = vfp_a8_neon_minmax((uint32_t)a, (uint32_t)b, minimum, &exceptions);
        uint32_t hi = vfp_a8_neon_minmax((uint32_t)(a >> 32), (uint32_t)(b >> 32), minimum, &exceptions);
        result[r] = ((uint64_t)hi << 32) | lo;
    }
    for (unsigned r = 0; r <= quad; r++) vfp_set_d(c, d + r, result[r]);
    c->vfp_fpscr |= exceptions;
...
/* VMLA/VMLS.F32 A1/T1 (DDI0406C.b A8.8.337), including reserved sz. */
static bool a8_neon_macc_space(const arm_cpu_t *c, uint32_t insn) {
    uint32_t prefix = (c->cpsr & ARM_CPSR_T) ? 0xef000d10u : 0xf2000d10u;
    return c->arch == ARM_ARCH_V7_CORTEX_A8 && (insn & 0xff800f10u) == prefix;
}

static arm_status_t exec_a8_neon_macc(arm_cpu_t *c, uint32_t insn) {
    unsigned d = ((insn >> 12) & 15u) | ((insn >> 18) & 16u);
    unsigned n = ((insn >> 16) & 15u) | ((insn >> 3) & 16u);
    unsigned m = (insn & 15u) | ((insn >> 1) & 16u), quad = (insn >> 6) & 1u;
    if ((insn & (1u << 20)) || (quad && ((d | n | m) & 1u))) return ARM_UNDEFINED;
    ...
    uint64_t result[2]; uint32_t exceptions = 0u;
    for (unsigned r = 0; r <= quad; r++) {
        uint64_t a = vfp_get_d(c, n + r), b = vfp_get_d(c, m + r), accumulator = vfp_get_d(c, d + r);
        /* The product is rounded/flushed first. VMLS negates that rounded
         * product before the separately rounded sum; this is not FMA. */
        uint32_t lo = a8_neon_multiply_f32((uint32_t)a, (uint32_t)b, &exceptions);
        uint32_t hi = a8_neon_multiply_f32((uint32_t)(a >> 32), (uint32_t)(b >> 32), &exceptions);
...
static bool vfp_lazy_enable_trap(const arm_cpu_t *c, uint32_t insn) {
    /* A8 SIMD data operations use checked access returns. The legacy broad
     * A32 SIMD mask must not turn an unsupported neighboring allocation into
     * a guest-enable retry just because CPACR/FPEXC currently denies access. */
    if (c->arch == ARM_ARCH_V7_CORTEX_A8 && !(c->cpsr & ARM_CPSR_T) &&
        (insn & 0xfe000000u) == 0xf2000000u) return false;
    ...
    if (a8_neon_single_elements_space(c, insn) || a8_neon_pairs_space(c, insn) || a8_neon_lane_load_space(c, insn) || a8_neon_bitwise_space(c, insn) ||
        ...
        a8_neon_minmax_space(c, insn)) return false;
...
static arm_status_t thumb32_step(arm_cpu_t *c, uint32_t pc, uint16_t first,
    ...
    if (a8_neon_lane_load_space(c, insn)) return exec_a8_neon_lane_load(c, insn);
    ...
    if (cond == 0xfu) {
        ...
        if (a8_neon_bitwise_space(c, insn) || a8_neon_immediate_space(c, insn) ||
            ...
            arm_status_t status = a8_neon_bitwise_space(c, insn) ? exec_a8_neon_bitwise(c, insn) :
                ...
                a8_neon_minmax_space(c, insn) ? exec_a8_neon_minmax(c, insn) : exec_a8_neon_macc(c, insn);
            if (status == ARM_GUEST_UNDEFINED) return take_undefined_instruction(c, pc);
            if (status != ARM_OK) return status;
            c->r[15] = next;
            return ARM_OK;
        }
        /* D15/D31 occupy the same field as the broad hint decoder's Rd=PC.
        ...
        if (c->arch == ARM_ARCH_V7_CORTEX_A8 && (insn & 0xff100000u) == 0xf4000000u) {
            if (!a8_neon_single_elements_space(c, insn) && !a8_neon_pairs_space(c, insn) && !a8_neon_lane_load_space(c, insn))
                return ARM_UNDEFINED;
            arm_status_t status = a8_neon_pairs_space(c, insn) ? exec_a8_neon_pairs(c, insn) :
                a8_neon_lane_load_space(c, insn) ? exec_a8_neon_lane_load(c, insn) : exec_a8_neon_single_elements(c, insn);
            if (c->abort_pending) return take_pending_data_abort(c, pc);
            if (arm_bus_access_failed(c->bus)) return halt_failed_instruction(c);
            if (status == ARM_GUEST_UNDEFINED) return take_undefined_instruction(c, pc);
            if (status != ARM_OK) return status;
