 ...
 * first integration deliberately refuses every memory instruction: guest
 * loads/stores need MMU, fault, MMIO and host-observer semantics that the flat
 * benchmark contract does not provide. A zero return is always a clean request
...
static void decode_entry(static_a64_entry_t *entry, const arm_cpu_t *cpu,
    ...
    /* Decode the longest exact prefix. A supported load/store still falls
     * back: shortening lets a preceding ALU prefix remain useful without
     * pretending the benchmark's flat masked RAM is an MMU. */
    for (unsigned count = candidate_insns; count != 0u; count--) {
        a64_static_block_t block;
        if (a64_static_decode_bytes_at(bytes, count, thumb, pc, &block) &&
...
unsigned s5l8900_static_a64_try(s5l8900_t *m, unsigned max_insns) {
    ...
    if (!a64_static_run(cpu, &entry->block, 1u, m->ram, m->ram_size)) {
         ...
         * a64_static_run refuses a block. */
        entry->valid = false;
        return 0u;
    }
    state->retired += entry->block.insn_count;
    return entry->block.insn_count;
#else
    (void)m;
    (void)max_insns;
    return 0u;
