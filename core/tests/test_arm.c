     ...
     for (unsigned lane=0;lane<2u;lane++) for (unsigned align=0;align<2u;align++)
      for (unsigned post=0;post<3u;post++) for (unsigned type=0;type<3u;type++)
       for (unsigned fault=0;fault<5u;fault++) {
        ...
        m_w32(NULL,0x4000u,0x6001u); m_w32(NULL,0x6000u,0x803eu);
        m_w32(NULL,0x6004u,fault==1u || fault==3u ? 0u : (fault==2u ? 0xa012u : fault==4u ? 0xa232u : 0xa032u)|types[type]);
        ...
        uint32_t flags=c.cpsr,stored=(uint32_t)(upper[15]>>(32u*lane));
        bool aborted=fault!=0u && (fault!=4u || !load);
        CHECK(arm_step(&c)==ARM_OK && c.cycles==1u && c.vfp_fpscr==0x0bc00080u,"lane MMU disposition");
        if (aborted) CHECK(c.r[15]==ARM_VEC_DATA_ABORT && c.r[14]==8u && c.spsr[ARM_BANK_ABT]==flags && c.r[4]==base &&
            c.cp15.dfar==base && c.cp15.dfsr==((fault==1u ? ARM_FSR_PAGE_TRANSLATION : fault==3u ? 1u : ARM_FSR_PAGE_PERMISSION)|(load ? 0u : 1u<<11)) &&
    ...
    for (unsigned host=0;host<2u;host++) for (unsigned load=0;load<2u;load++) for (unsigned lane=0;lane<2u;lane++)
     for (unsigned align=0;align<2u;align++) for (unsigned fault=0;fault<4u;fault++) {
        ...
        if (fault) CHECK(c.r[15]==ARM_VEC_PREFETCH && c.r[14]==0x1002u && c.spsr[ARM_BANK_ABT]==flags &&
            ...
            "lane availability/transfer preceded full User fetch");
        else {
        ...
        g_watch_addr=UINT32_MAX;
        CHECK(m_r32(NULL,0xc000u)==(fault || load ? 0x7f800001u : stored),"lane split fetch changed uncompleted RAM");
     }
}
    ...
    for (unsigned thumb=0;thumb<2u;thumb++) for (unsigned load=0;load<2u;load++)
     for (unsigned lane=0;lane<2u;lane++) for (unsigned align=0;align<2u;align++) {
        ...
        CHECK(arm_step(&c)==ARM_OK && c.r[15]==ARM_VEC_UNDEFINED && c.r[14]==pc+(thumb ? 2u : 4u) &&
              c.spsr[ARM_BANK_UND]==flags && c.r[4]==0x2000u && c.vfp_fpscr==0x0bc00080u && g_watch_reads32==0u && g_watch_writes32==0u &&
        ...
        CHECK(arm_step(&c)==ARM_OK && c.r[15]==0x108u && c.r[2]==1u && c.cpsr==(flags&~TEST_IT_MASK) &&
              c.cycles==(thumb ? 6u : 5u) && g_watch_reads32==load && g_watch_writes32==!load,"lane retry changed following condition");
        g_watch_addr=UINT32_MAX;
        CHECK(m_r32(NULL,0x2000u)==(load ? 0x7f800001u : stored),"lane guest retry stored wrong lane");
    }
}
     ...
     for (unsigned thumb = 0; thumb < 2u; thumb++)
      for (unsigned load = 0; load < 2u; load++)
       for (unsigned kind = 0; kind < 3u; kind++)
        for (unsigned prefix = 0; prefix < (kind == 2u ? 8u : 4u); prefix++)
         for (unsigned fault = 0; fault < 3u; fault++) {
            memset(g_ram, 0xee, sizeof g_ram);
            arm_bus_t bus = g_bus;
            if (host) { bus.host_ram = m_host_ram; bus.host_ram_write = m_host_ram_write; }
            arm_cpu_t c;
            CHECK(arm_reset_profile(&c, &bus, ARM_ARCH_V7_CORTEX_A8), "pair MMU reset");
            c.cp15.sctlr = ARM_SCTLR_M | ARM_SCTLR_XP;
            c.cp15.ttbr0 = 0x4000u; c.cp15.dacr = 1u; c.cp15.cpacr = 0x00f00000u;
            c.vfp_fpexc = ARM_FPEXC_EN; c.vfp_fpscr = 0x0bc00080u;
            c.cpsr = ARM_MODE_USR | ARM_CPSR_N | ARM_CPSR_C | (thumb ? ARM_CPSR_T | test_it_bits(0x18u) : 0u);
            uint32_t base = 0x2000u - prefix * 4u; c.r[4] = base;
            uint32_t expected[8], physical[8];
            for (unsigned word = 0; word < 8u; word++) expected[word] = 0xdead0000u + word;
            for (unsigned d = 0; d < 4u; d++) c.a8_vfp_hi[12u + d] = (uint64_t)expected[2u * d + 1u] << 32 | expected[2u * d];
            m_w32(NULL, 0x4000u, 0x6001u); m_w32(NULL, 0x6000u, 0x803eu);
            m_w32(NULL, 0x6004u, 0xa03eu); m_w32(NULL, 0x6008u, fault == 1u ? 0u : fault == 2u ? 0xc01eu : 0xc03eu);
            uint32_t insn = (thumb ? 0xf944c08du : 0xf444c08du) | (load << 21) | (types[kind] << 8);
            if (thumb) { m_w16(NULL, 0x8000u, (uint16_t)(insn >> 16)); m_w16(NULL, 0x8002u, (uint16_t)insn); }
            else m_w32(NULL, 0x8000u, insn);
            for (unsigned word = 0; word < 8u; word++) {
                uint32_t va = base + word * 4u;
                physical[word] = (va < 0x2000u ? 0xa000u : 0xc000u) + (va & 0xfffu);
                m_w32(NULL, physical[word], 0xabcdef00u + word);
            }
            g_watch_addr = 0xc000u; g_watch_reads32 = g_watch_writes32 = 0u;
            uint32_t flags = c.cpsr; unsigned words = kind == 2u ? 8u : 4u;
            CHECK(arm_step(&c) == ARM_OK && c.cycles == 1u && c.vfp_fpscr == 0x0bc00080u, "pair MMU disposition");
            unsigned completed = fault ? prefix : words;
            if (fault) CHECK(c.r[15] == ARM_VEC_DATA_ABORT && c.r[14] == 8u && c.spsr[ARM_BANK_ABT] == flags &&
                c.r[4] == base && c.cp15.dfar == 0x2000u &&
                c.cp15.dfsr == ((fault == 1u ? ARM_FSR_PAGE_TRANSLATION : ARM_FSR_PAGE_PERMISSION) | (load ? 0u : 0x800u)) &&
                !(c.cpsr & TEST_IT_MASK) && !g_watch_reads32 && !g_watch_writes32,
                "pair MMU first fault/base/IT or physical access after refusal");
            else CHECK(c.r[15] == 4u && c.r[4] == base + words * 4u && c.cpsr == (flags & ~TEST_IT_MASK), "pair MMU success/IT");
            g_watch_addr = UINT32_MAX;
            for (unsigned word = 0; word < 8u; word++) {
                if (load && word < completed) expected[slots[kind][word]] = 0xabcdef00u + word;
                CHECK(m_r32(NULL, physical[word]) == (!load && word < completed ? 0xdead0000u + slots[kind][word] : 0xabcdef00u + word),
                      "pair MMU store prefix/order/suffix");
            }
            for (unsigned word = 0; word < 8u; word++) CHECK((uint32_t)(c.a8_vfp_hi[12u + word / 2u] >> (32u * (word % 2u))) == expected[word],
                "pair MMU load prefix/order/gap preservation");
         }
    for (unsigned host = 0; host < 2u; host++)
     for (unsigned load = 0; load < 2u; load++)
      for (unsigned kind = 0; kind < 3u; kind++)
       for (unsigned fault = 1u; fault <= 3u; fault++) {
        ...
        arm_bus_t bus = g_bus; if (host) bus.host_ram = m_host_ram;
        arm_cpu_t c;
        CHECK(arm_reset_profile(&c, &bus, ARM_ARCH_V7_CORTEX_A8), "pair split fetch reset");
        c.cp15.sctlr = ARM_SCTLR_M | ARM_SCTLR_XP; c.cp15.ttbr0 = 0x4000u; c.cp15.dacr = 1u;
        c.cpsr = ARM_MODE_USR | ARM_CPSR_T | ARM_CPSR_N | test_it_bits(0x1cu);
        c.r[15] = 0xffeu; c.r[4] = 0x3000u;
        c.a8_vfp_hi[15] = UINT64_C(0x123456789abcdef0); c.vfp_fpscr = 0x0bc00080u;
        uint32_t insn = 0xf944c08du | (load << 21) | (types[kind] << 8), flags = c.cpsr;
        m_w32(NULL, 0x4000u, 0x6001u); m_w32(NULL, 0x6000u, 0x8032u);
        m_w32(NULL, 0x6004u, fault == 1u ? 0u : fault == 2u ? 0xa033u : 0xa012u);
        m_w16(NULL, 0x8ffeu, (uint16_t)(insn >> 16)); m_w16(NULL, 0xa000u, (uint16_t)insn);
        CHECK(arm_step(&c) == ARM_OK && c.r[15] == ARM_VEC_PREFETCH && c.r[14] == 0x1002u &&
              ...
              (c.cp15.ifsr & 15u) == (fault == 1u ? ARM_FSR_PAGE_TRANSLATION : ARM_FSR_PAGE_PERMISSION) &&
              c.r[4] == 0x3000u && c.vfp_fpscr == 0x0bc00080u && c.vfp_fpexc == 0u &&
              c.a8_vfp_hi[15] == UINT64_C(0x123456789abcdef0), "pair availability/effects preceded full Thumb fetch");
       }
}
     ...
     for (unsigned kind=0;kind<3u;kind++)
      for (unsigned quad=0;quad<2u;quad++)
       for (unsigned index=0;index<2u;index++)
        for (unsigned fault=0;fault<4u;fault++) {
            memset(g_ram,0,sizeof g_ram); arm_bus_t bus=g_bus; if (host) bus.host_ram=m_host_ram;
            arm_cpu_t c; CHECK(arm_reset_profile(&c,&bus,ARM_ARCH_V7_CORTEX_A8),"scalar split fetch reset");
            c.cp15.sctlr=ARM_SCTLR_M|ARM_SCTLR_XP; c.cp15.ttbr0=0x4000u; c.cp15.dacr=1u; c.cp15.cpacr=0x00f00000u;
            c.cpsr=ARM_MODE_USR|ARM_CPSR_T|ARM_CPSR_N|test_it_bits(0x1cu);
            c.r[15]=0xffeu; c.vfp_fpscr=0x0bc00080u; c.vfp_fpexc=fault ? 0u : ARM_FPEXC_EN;
            c.a8_vfp_hi[14]=c.a8_vfp_hi[15]=accumulator; c.a8_vfp_hi[0]=c.a8_vfp_hi[1]=a;
            c.vfp_s[30]=index ? 0x7f812345u : 0x40000000u; c.vfp_s[31]=index ? 0x40000000u : 0x7f812345u;
            uint32_t insn=0xefe0e1cfu|(kind<<10)|(quad<<28)|(index<<5), flags=c.cpsr;
            m_w32(NULL,0x4000u,0x6001u); m_w32(NULL,0x6000u,0x8032u);
            m_w32(NULL,0x6004u,fault==1u ? 0u : fault==2u ? 0xa033u : fault==3u ? 0xa012u : 0xa032u);
            m_w16(NULL,0x8ffeu,(uint16_t)(insn>>16)); m_w16(NULL,0xa000u,(uint16_t)insn); m_w16(NULL,0x9000u,0u);
            CHECK(arm_step(&c)==ARM_OK && c.cycles==1u && c.vfp_fpscr==0x0bc00080u,"scalar split fetch disposition");
            if (fault) CHECK(c.r[15]==ARM_VEC_PREFETCH && c.r[14]==0x1002u && c.spsr[ARM_BANK_ABT]==flags &&
                c.cp15.ifar==0x1000u && (c.cp15.ifsr&15u)==(fault==1u ? ARM_FSR_PAGE_TRANSLATION : ARM_FSR_PAGE_PERMISSION) &&
                !(c.cpsr&(ARM_CPSR_T|TEST_IT_MASK)) && c.vfp_fpexc==0u && c.a8_vfp_hi[14]==accumulator && c.a8_vfp_hi[15]==accumulator,
                "scalar access/arithmetic preceded complete fetch");
            else CHECK(c.r[15]==0x1002u && c.cpsr==((flags&~TEST_IT_MASK)|test_it_bits(0x18u)) &&
                c.a8_vfp_hi[14]==results[kind] && c.a8_vfp_hi[15]==(quad ? results[kind] : accumulator),"scalar split fetch result/IT");
            CHECK(c.a8_vfp_hi[0]==a && c.a8_vfp_hi[1]==a && c.vfp_s[30]==(index ? 0x7f812345u : 0x40000000u) &&
                c.vfp_s[31]==(index ? 0x40000000u : 0x7f812345u),"scalar split fetch source preservation");
        }
    for (unsigned kind=0;kind<3u;kind++)
     for (unsigned quad=0;quad<2u;quad++)
      for (unsigned index=0;index<2u;index++) {
        memset(g_ram,0,sizeof g_ram); arm_cpu_t c;
        CHECK(arm_reset_profile(&c,&g_bus,ARM_ARCH_V7_CORTEX_A8),"scalar lazy retry reset");
        c.cpsr=ARM_MODE_USR|ARM_CPSR_T|ARM_CPSR_Z|ARM_CPSR_Q; c.cp15.cpacr=0x00f00000u;
        c.a8_vfp_hi[14]=c.a8_vfp_hi[15]=accumulator; c.a8_vfp_hi[0]=c.a8_vfp_hi[1]=a;
        c.vfp_s[30]=index ? 0x7f812345u : 0x40000000u; c.vfp_s[31]=index ? 0x40000000u : 0x7f812345u;
        c.r[15]=0x100u; c.r[5]=ARM_FPEXC_EN; c.vfp_fpscr=0x0bc00080u;
        uint32_t insn=0xefe0e1cfu|(kind<<10)|(quad<<28)|(index<<5);
        m_w16(NULL,0x100u,0xbf04u); m_w16(NULL,0x102u,(uint16_t)(insn>>16)); m_w16(NULL,0x104u,(uint16_t)insn); m_w16(NULL,0x106u,0x2201u);
        put_vfp_system_transfer(0u,ARM_VEC_UNDEFINED,0u,8u,5u); m_w32(NULL,8u,0xe25ef002u);
        CHECK(arm_step(&c)==ARM_OK,"scalar lazy IT setup"); uint32_t flags=c.cpsr;
        CHECK(arm_step(&c)==ARM_OK && c.r[15]==ARM_VEC_UNDEFINED && c.r[14]==0x104u && c.spsr[ARM_BANK_UND]==flags &&
              c.a8_vfp_hi[14]==accumulator && c.a8_vfp_hi[15]==accumulator,"scalar arithmetic preceded guest enable");
        CHECK(arm_step(&c)==ARM_OK && c.vfp_fpexc==ARM_FPEXC_EN,"scalar handler enable");
        CHECK(arm_step(&c)==ARM_OK && c.r[15]==0x102u && c.cpsr==flags,"scalar exception return");
        CHECK(arm_step(&c)==ARM_OK && c.r[15]==0x106u && c.cpsr==((flags&~TEST_IT_MASK)|test_it_bits(0x08u)) &&
              c.vfp_fpscr==0x0bc00080u && c.a8_vfp_hi[14]==results[kind] && c.a8_vfp_hi[15]==(quad ? results[kind] : accumulator),
              "scalar guest retry did not execute exactly once");
        CHECK(arm_step(&c)==ARM_OK && c.r[15]==0x108u && c.r[2]==1u && c.cpsr==(flags&~TEST_IT_MASK) && c.cycles==6u,
              "scalar retry changed following IT condition");
      }
}
...
static void test_cortex_a8_vfp_binary_fetch_and_retry(void) {
    ...
    for (unsigned dbl = 0; dbl < 2u; dbl++)
     for (unsigned op = 0; op < 6u; op++) {
        ...
        m_w16(NULL,0x100u,0xbf04u); /* ITT EQ */
        m_w16(NULL,0x102u,(uint16_t)(insns[dbl][op]>>16)); m_w16(NULL,0x104u,(uint16_t)insns[dbl][op]);
        m_w16(NULL,0x106u,0x2201u);
        put_vfp_system_transfer(0u,ARM_VEC_UNDEFINED,0u,8u,5u); m_w32(NULL,8u,0xe25ef002u);
...
static void test_cortex_a8_vfp_precision_fetch_and_retry(void) {
    ...
    for (unsigned host = 0; host < 2u; host++)
     for (unsigned narrow = 0; narrow < 2u; narrow++)
      for (unsigned fault = 0; fault < 4u; fault++) {
        memset(g_ram,0,sizeof g_ram);
        arm_bus_t bus = g_bus; if (host) bus.host_ram = m_host_ram;
        arm_cpu_t c; CHECK(arm_reset_profile(&c,&bus,ARM_ARCH_V7_CORTEX_A8),"reset precision fetch");
        c.cp15.sctlr = ARM_SCTLR_M | ARM_SCTLR_XP; c.cp15.ttbr0 = 0x4000u; c.cp15.dacr = 1u;
        c.cp15.cpacr = 0x00f00000u; c.vfp_fpexc = fault ? 0u : 0x40000000u; c.vfp_fpscr = 0x4bc00080u;
        c.cpsr = ARM_MODE_USR | ARM_CPSR_T | ARM_CPSR_N | test_it_bits(0x1cu); c.r[15] = 0xffeu;
        for (unsigned s = 0; s < 32u; s++) c.vfp_s[s] = 0xdead0000u+s;
        for (unsigned d = 0; d < 16u; d++) c.a8_vfp_hi[d] = UINT64_C(0x7ff01234beef0000)+d;
        c.vfp_s[16] = 0x7f800001u; c.a8_vfp_hi[0] = UINT64_C(0x3ff0000030000000);
        uint32_t singles[32], flags = c.cpsr; uint64_t upper[16];
        memcpy(singles,c.vfp_s,sizeof singles); memcpy(upper,c.a8_vfp_hi,sizeof upper);
        m_w32(NULL,0x4000u,0x6001u); m_w32(NULL,0x6000u,0x8032u);
        m_w32(NULL,0x6004u,fault == 1u ? 0u : fault == 2u ? 0xa033u : fault == 3u ? 0xa012u : 0xa032u);
        m_w16(NULL,0x8ffeu,(uint16_t)(insns[narrow]>>16)); m_w16(NULL,0xa000u,(uint16_t)insns[narrow]);
        m_w16(NULL,0x9000u,0u);
        CHECK(arm_step(&c) == ARM_OK && c.cycles == 1u &&
              c.vfp_fpscr == (0x4bc00080u | (fault ? 0u : narrow ? 0x10u : 0x1u)),"precision split fetch disposition");
        if (!fault) {
            if (narrow) singles[31] = 0x3f800001u; else upper[15] = UINT64_C(0x7ff8000000000000);
            CHECK(c.r[15] == 0x1002u && c.cpsr == ((flags & ~TEST_IT_MASK) | test_it_bits(0x18u)),"precision split fetch/IT");
        } else CHECK(c.r[15] == ARM_VEC_PREFETCH && c.r[14] == 0x1002u && c.cp15.ifar == 0x1000u &&
            c.spsr[ARM_BANK_ABT] == flags && !(c.cpsr & (ARM_CPSR_T | TEST_IT_MASK)) && c.vfp_fpexc == 0u &&
            (c.cp15.ifsr & 15u) == (fault == 1u ? ARM_FSR_PAGE_TRANSLATION : ARM_FSR_PAGE_PERMISSION),
            "precision availability/effects preceded second-half User fetch");
        CHECK(memcmp(singles,c.vfp_s,sizeof singles) == 0 && memcmp(upper,c.a8_vfp_hi,sizeof upper) == 0,
              "precision split fetch register result/preservation");
      }
    for (unsigned narrow = 0; narrow < 2u; narrow++) {
        arm_cpu_t c; CHECK(arm_reset_profile(&c,&g_bus,ARM_ARCH_V7_CORTEX_A8),"reset precision retry");
        c.cpsr = ARM_MODE_USR | ARM_CPSR_T | ARM_CPSR_Z | ARM_CPSR_Q;
        c.cp15.cpacr = 0x00f00000u; c.vfp_fpscr = 0x08000002u;
        c.vfp_s[16] = 0x7f800001u; c.a8_vfp_hi[0] = UINT64_C(0x3ff0000030000000);
        uint32_t singles[32]; uint64_t upper[16];
        ...
        c.r[15] = 0x100u; c.r[5] = 0x40000000u; c.r[2] = 0x12345678u;
        m_w16(NULL,0x100u,0xbf04u); /* ITT EQ */
        m_w16(NULL,0x102u,(uint16_t)(insns[narrow]>>16)); m_w16(NULL,0x104u,(uint16_t)insns[narrow]);
        m_w16(NULL,0x106u,0x2201u);
        put_vfp_system_transfer(0u,ARM_VEC_UNDEFINED,0u,8u,5u); m_w32(NULL,8u,0xe25ef002u);
        CHECK(arm_step(&c) == ARM_OK,"precision retry IT setup");
        uint32_t interrupted = c.cpsr;
        CHECK(arm_step(&c) == ARM_OK && c.r[15] == ARM_VEC_UNDEFINED && c.r[14] == 0x104u &&
              c.spsr[ARM_BANK_UND] == interrupted && c.vfp_fpscr == 0x08000002u &&
              memcmp(singles,c.vfp_s,sizeof singles) == 0 && memcmp(upper,c.a8_vfp_hi,sizeof upper) == 0,
              "precision lazy exception changed FP state");
        CHECK(arm_step(&c) == ARM_OK && c.vfp_fpexc == 0x40000000u,"precision guest enable");
        CHECK(arm_step(&c) == ARM_OK && c.r[15] == 0x102u && c.cpsr == interrupted,"precision exception return");
        /* The guest retry enables VFP with RN and DN clear: preserve the NaN
         * payload, or round the halfway finite input to the even result. */
        if (narrow) singles[31] = 0x3f800002u; else upper[15] = UINT64_C(0x7ff8000020000000);
        CHECK(arm_step(&c) == ARM_OK && c.r[15] == 0x106u &&
              c.vfp_fpscr == (0x08000002u | (narrow ? 0x10u : 0x1u)) &&
              c.cpsr == ((interrupted & ~TEST_IT_MASK) | test_it_bits(0x08u)) &&
              memcmp(singles,c.vfp_s,sizeof singles) == 0 && memcmp(upper,c.a8_vfp_hi,sizeof upper) == 0,
              "precision exact retry/IT/register state");
        CHECK(arm_step(&c) == ARM_OK && c.r[15] == 0x108u && c.r[2] == 1u &&
              c.cpsr == (interrupted & ~TEST_IT_MASK) && c.cycles == 6u,"precision changed following IT condition");
    }
}
...
static void test_cortex_a8_vfp_data_fetch_and_retry(void) {
    static const uint32_t insns[] = {
        ...
        0xfff9f720u,0xfff9f7a0u,0xfff9e760u,0xfff9e7e0u
    };
    ...
    static const uint64_t neon_fetch_values[] = {
        ...
        UINT64_C(0x7ff0000000000001),UINT64_C(0x7ff0000080000001),0u,UINT64_C(0x8000000080000000)
    };
