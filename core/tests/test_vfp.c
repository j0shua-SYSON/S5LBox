...
/* VFP VADD/VSUB T2/A2, with actual S/D register numbers. */
static uint32_t a8_fp_add(unsigned sub, unsigned dbl, unsigned dst, unsigned left, unsigned right) {
    return 0xee300a00u | (sub << 6) | (dbl << 8) |
        (dbl ? ((dst & 15u) << 12) | ((dst >> 4) << 22) | ((left & 15u) << 16) |
               ((left >> 4) << 7) | (right & 15u) | ((right >> 4) << 5) :
               ((dst >> 1) << 12) | ((dst & 1u) << 22) | ((left >> 1) << 16) |
               ((left & 1u) << 7) | (right >> 1) | ((right & 1u) << 5));
}

static uint32_t a8_fp_multiply(unsigned neg, unsigned dbl, unsigned dst, unsigned left, unsigned right) {
...
/* Normal powers of two used for exact division register/vector oracles. */
static uint64_t a8_power_of_two(unsigned dbl, int exponent, unsigned negative) {
    return ((uint64_t)(exponent + (dbl ? 1023 : 127)) << (dbl ? 52u : 23u)) |
        (negative ? (dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000)) : 0u);
}

static uint64_t a8_add_integer(unsigned dbl, int value) {
    return dbl ? d2u((double)value) : f2u((float)value); /* Small exact integers only. */
}

static void a8_add_expected_set(uint64_t expected[32], unsigned dbl, unsigned reg, uint64_t value) {
    if (dbl) expected[reg] = value;
    else {
        unsigned shift = (reg % 2u) * 32u;
        expected[reg / 2u] = (expected[reg / 2u] & ~(UINT64_C(0xffffffff) << shift)) | (value << shift);
    }
}

static void test_a8_vfp_add_registers(void) {
    CHECK(a8_fp_add(0u,1u,18u,17u,16u) == 0xee712ba0u &&
          a8_fp_add(1u,0u,31u,16u,1u) == 0xee78fa60u, "VFP add/sub encoding anchors");
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned dbl = 0; dbl < 2u; dbl++)
       ...
       for (unsigned dst = 0; dst < 32u; dst++)
        for (unsigned left = 0; left < 32u; left++)
         for (unsigned right = 0; right < 32u; right++) {
            ...
            int sum = (int)left - 16 + (sub ? 16 - (int)right : (int)right - 16);
            a8_add_expected_set(expected, dbl, dst, a8_add_integer(dbl, sum));
            c.excl_valid = true; c.excl_addr = 0x12340u;
            uint32_t flags = c.cpsr, fpscr = c.vfp_fpscr;
            CHECK(a8_move_step(&c, thumb, a8_fp_add(sub,dbl,dst,left,right)) == ARM_OK &&
                  c.r[15] == 0x104u && c.cycles == 1u && c.cpsr == flags && c.vfp_fpscr == fpscr &&
                  c.excl_valid && c.excl_addr == 0x12340u, "VFP add register state T=%u D=%u sub=%u d/n/m=%u/%u/%u",
                  thumb,dbl,sub,dst,left,right);
            bool match = true;
            for (unsigned d = 0; d < 32u; d++) match &= vfp_get_d(&c,d) == expected[d];
            for (unsigned r = 0; r < 15u; r++) match &= c.r[r] == 0u;
            CHECK(match, "VFP add register/alias oracle");
         }
}

/* The finite-value oracle uses native IEEE operations and fenv, independent
 * of the production integer alignment/rounding. Raw analytical anchors below
 * check that the host actually honors the requested modes and exceptions. */
static uint64_t a8_add_native(uint64_t a, uint64_t b, unsigned dbl, unsigned sub,
    ...
    *flags = 0u;
    ...
    CHECK(fegetenv(&saved) == 0 && fesetround(rounds[(fpscr >> 22) & 3u]) == 0 &&
          feclearexcept(FE_ALL_EXCEPT) == 0, "prepare native add oracle");
    uint64_t bits;
    if (dbl) {
        volatile double x = u2d(a), y = u2d(b);
        volatile double z = sub ? x - y : x + y;
        bits = d2u(z);
    } else {
        volatile float x = u2f((uint32_t)a), y = u2f((uint32_t)b);
        volatile float z = sub ? x - y : x + y;
        bits = f2u(z);
    }
    int exceptions = fetestexcept(FE_ALL_EXCEPT);
    if (exceptions & FE_OVERFLOW) *flags |= ARM_FPSCR_OFC;
    if (exceptions & FE_UNDERFLOW) *flags |= ARM_FPSCR_UFC;
    if (exceptions & FE_INEXACT) *flags |= ARM_FPSCR_IXC;
    CHECK(!(exceptions & (FE_INVALID | FE_DIVBYZERO)), "nonfinite input reached finite add oracle");
    /* Addition/subtraction of same-format operands can only produce exact
     * subnormals. ARM FZ still reports UFC when flushing that nonzero result. */
    if ((fpscr & ARM_FPSCR_FZ) && (bits & ~sign) && (bits & ~sign) < normal) {
        bits &= sign; *flags |= ARM_FPSCR_UFC;
    }
    CHECK(fesetenv(&saved) == 0, "restore native add oracle host state");
    return bits;
}

static void test_a8_vfp_add_values_and_host_state(void) {
    fenv_t saved; CHECK(fegetenv(&saved) == 0, "save VFP add host state");
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
    for (unsigned dbl = 0; dbl < 2u; dbl++) {
        uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
        uint64_t normal = dbl ? UINT64_C(0x0010000000000000) : UINT64_C(0x00800000);
        uint64_t infinity = dbl ? UINT64_C(0x7ff0000000000000) : UINT64_C(0x7f800000);
        uint64_t one = dbl ? UINT64_C(0x3ff0000000000000) : UINT64_C(0x3f800000);
        uint64_t half_ulp = dbl ? UINT64_C(0x3ca0000000000000) : UINT64_C(0x33800000);
        uint64_t half_below = dbl ? UINT64_C(0x3c90000000000000) : UINT64_C(0x33000000);
        const struct { uint64_t a,b,result[4]; uint32_t flags; } anchors[] = {
            {0u,sign,{0u,0u,sign,0u},0u}, {sign,sign,{sign,sign,sign,sign},0u},
            {one,half_ulp,{one,one+1u,one,one},ARM_FPSCR_IXC},
            {one+1u,half_ulp,{one+2u,one+2u,one+1u,one+1u},ARM_FPSCR_IXC},
            {one,sign|half_below,{one,one,one-1u,one-1u},ARM_FPSCR_IXC},
            {one,1u,{one,one+1u,one,one},ARM_FPSCR_IXC},
            {sign|one,sign|half_ulp,{sign|one,sign|one,sign|(one+1u),sign|one},ARM_FPSCR_IXC},
            {one,sign|one,{0u,0u,sign,0u},0u},
            {normal,sign|(normal+1u),{sign|1u,sign|1u,sign|1u,sign|1u},0u},
            {normal,sign|(normal-1u),{1u,1u,1u,1u},0u},
            {normal-1u,1u,{normal,normal,normal,normal},0u},
            {1u,1u,{2u,2u,2u,2u},0u},
            {infinity-1u,infinity-1u,{infinity,infinity,infinity-1u,infinity-1u},ARM_FPSCR_OFC|ARM_FPSCR_IXC},
            {sign|(infinity-1u),sign|(infinity-1u),{sign|infinity,sign|(infinity-1u),sign|infinity,sign|(infinity-1u)},ARM_FPSCR_OFC|ARM_FPSCR_IXC}
        };
        for (unsigned row = 0; row < sizeof anchors / sizeof anchors[0]; row++)
         for (unsigned mode = 0; mode < 4u; mode++)
          for (unsigned sub = 0; sub < 2u; sub++) {
            uint32_t exceptions;
            uint64_t b = anchors[row].b ^ (sub ? sign : 0u);
            uint64_t result = a8_add_native(anchors[row].a,b,dbl,sub,mode<<22,&exceptions);
            CHECK(result == anchors[row].result[mode] && exceptions == anchors[row].flags,
                  "native add anchor D=%u row=%u mode=%u sub=%u",dbl,row,mode,sub);
            for (unsigned thumb = 0; thumb < 2u; thumb++) {
                arm_cpu_t c; a8_move_reset(&c,thumb);
                c.vfp_fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC | ARM_FPSCR_DZC | (mode<<22);
                a8_fp_value_set(&c,dbl,16u,anchors[row].a); a8_fp_value_set(&c,dbl,1u,b);
                uint32_t flags = c.vfp_fpscr;
                CHECK(a8_move_step(&c,thumb,a8_fp_add(sub,dbl,31u,16u,1u)) == ARM_OK &&
                      a8_fp_value(&c,dbl,31u) == anchors[row].result[mode] &&
                      c.vfp_fpscr == (flags | anchors[row].flags), "VFP add analytical anchor");
            }
          }
        uint64_t random = UINT64_C(0xd12a3b4c5d6e7f89);
        for (unsigned sample = 0; sample < 1024u; sample++) {
            random ^= random << 13; random ^= random >> 7; random ^= random << 17;
            uint64_t a = dbl ? random : (uint32_t)random;
            random ^= random << 13; random ^= random >> 7; random ^= random << 17;
            uint64_t b = dbl ? random : (uint32_t)random;
            if ((a & ~sign) >= infinity) a &= ~infinity;
            if ((b & ~sign) >= infinity) b &= ~infinity;
            /* Include cancellation, close exponents, denormals and very
             * large exponent gaps, in addition to the independent samples. */
            if (sample % 8u == 0u) b = a ^ sign;
            if (sample % 8u == 1u) b = (b & (sign | (normal-1u))) | (a & infinity);
            if (sample % 8u == 2u) a &= sign | (normal-1u);
            if (sample % 8u == 3u) { a = normal; b = sign | (normal+1u); }
            for (unsigned controls = 0; controls < 16u; controls++)
             for (unsigned sub = 0; sub < 2u; sub++) {
                uint32_t fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC | ARM_FPSCR_DZC |
                    ((controls & 3u) << 22) | (controls & 4u ? ARM_FPSCR_FZ : 0u) |
                    (controls & 8u ? ARM_FPSCR_DN : 0u), exceptions;
                uint64_t want = a8_add_native(a,b,dbl,sub,fpscr,&exceptions);
                for (unsigned thumb = 0; thumb < 2u; thumb++) {
                    arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr = fpscr;
                    a8_fp_value_set(&c,dbl,16u,a); a8_fp_value_set(&c,dbl,1u,b);
                    unsigned host = sample % 4u;
                    CHECK(fesetround(rounds[host]) == 0 && feclearexcept(FE_ALL_EXCEPT) == 0 &&
                          (!(sample & 1u) || feraiseexcept(FE_INVALID | FE_DIVBYZERO) == 0), "prepare VFP add host state");
                    int pending = fetestexcept(FE_ALL_EXCEPT);
                    uint32_t cpsr = c.cpsr;
                    CHECK(a8_move_step(&c,thumb,a8_fp_add(sub,dbl,31u,16u,1u)) == ARM_OK &&
                          a8_fp_value(&c,dbl,31u) == want && c.vfp_fpscr == (fpscr | exceptions) && c.cpsr == cpsr,
                          "VFP finite oracle D=%u T=%u sub=%u controls=%u sample=%u",dbl,thumb,sub,controls,sample);
                    CHECK(fegetround() == rounds[host] && fetestexcept(FE_ALL_EXCEPT) == pending,
                          "VFP add changed host rounding or exceptions");
                }
             }
        }
    }
    CHECK(fesetenv(&saved) == 0, "restore VFP add host state");
}

static void test_a8_vfp_add_special_values(void) {
    fenv_t saved; CHECK(fegetenv(&saved) == 0, "save special add host state");
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
       ...
       for (unsigned controls = 0; controls < 16u; controls++)
        for (unsigned x = 0; x < count; x++)
         for (unsigned y = 0; y < count; y++) {
            bool nan = a8_compare_values[x].nan || a8_compare_values[y].nan;
            bool inf_x = x == 0u || x == 17u, inf_y = y == 0u || y == 17u;
            if (!nan && !inf_x && !inf_y) continue;
            uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
            uint64_t infinity = dbl ? UINT64_C(0x7ff0000000000000) : UINT64_C(0x7f800000);
            ...
            uint32_t exceptions = (controls & 4u) &&
                (a8_compare_values[x].denormal || a8_compare_values[y].denormal) ? ARM_FPSCR_IDC : 0u;
            uint64_t want;
            if (nan) {
                unsigned chosen = a8_compare_values[x].signaling ? x : a8_compare_values[y].signaling ? y :
                                  a8_compare_values[x].nan ? x : y;
                want = (controls & 8u) ? infinity | quiet :
                    (dbl ? a8_compare_values[chosen].dual : a8_compare_values[chosen].single) | quiet;
                if (a8_compare_values[x].signaling || a8_compare_values[y].signaling) exceptions |= ARM_FPSCR_IOC;
            } else if (inf_x && inf_y && (((a ^ b) & sign) != 0u) != (sub != 0u)) {
                want = infinity | quiet; exceptions |= ARM_FPSCR_IOC;
            } else want = inf_x ? a : b ^ (sub ? sign : 0u);
            arm_cpu_t c; a8_move_reset(&c,thumb);
            c.vfp_fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC | ARM_FPSCR_DZC |
                ((controls & 3u) << 22) | (controls & 4u ? ARM_FPSCR_FZ : 0u) |
                (controls & 8u ? ARM_FPSCR_DN : 0u);
            a8_fp_value_set(&c,dbl,16u,a); a8_fp_value_set(&c,dbl,1u,b);
            uint32_t flags = c.vfp_fpscr;
            unsigned host = (x + y) % 4u;
            CHECK(fesetround(rounds[host]) == 0 && feclearexcept(FE_ALL_EXCEPT) == 0 &&
                  (!((x + y) & 1u) || feraiseexcept(FE_DIVBYZERO) == 0), "prepare special add host state");
            int pending = fetestexcept(FE_ALL_EXCEPT);
            CHECK(a8_move_step(&c,thumb,a8_fp_add(sub,dbl,31u,16u,1u)) == ARM_OK &&
                  a8_fp_value(&c,dbl,31u) == want && c.vfp_fpscr == (flags | exceptions),
                  "VFP NaN/infinity D=%u T=%u sub=%u controls=%u x/y=%u/%u",dbl,thumb,sub,controls,x,y);
            CHECK(fegetround() == rounds[host] && fetestexcept(FE_ALL_EXCEPT) == pending,
                  "special VFP add changed host rounding or exceptions");
         }
    CHECK(fesetenv(&saved) == 0, "restore special add host state");
}
     ...
     for (unsigned dbl = 0; dbl < 2u; dbl++)
      for (unsigned op = 0; op < 8u; op++)
       for (unsigned len = 0; len < 8u; len++)
        for (unsigned stride = 0; stride < 4u; stride++)
         for (unsigned dst = 0; dst < 32u; dst++) {
            unsigned bank = dbl ? 4u : 8u, step = stride == 3u ? 2u : 1u;
            bool scalar = !len || (dbl ? dst % 16u < 4u : dst < 8u);
            unsigned count = scalar ? 1u : len + 1u;
            bool valid = (stride == 0u || (stride == 3u && len >= 1u && len <= 3u)) &&
                         (scalar || count * step <= bank);
            for (unsigned left = 0; left < (valid ? 32u : 1u); left++) {
                unsigned right = (left * 7u + dst) % 32u;
                arm_cpu_t c; a8_move_reset(&c,thumb);
                c.vfp_fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC | ARM_FPSCR_DZC | (len<<16) | (stride<<20);
                ...
                if (valid) for (unsigned lane = 0; lane < count; lane++) {
                    unsigned dr = dst / bank * bank + (dst % bank + lane*step) % bank;
                    unsigned nr = left / bank * bank + (left % bank + lane*step) % bank;
                ...
                uint32_t flags = c.cpsr, fpscr = c.vfp_fpscr;
                uint32_t insn = a8_fp_binary(op,dbl,dst,left,right);
                CHECK(a8_move_step(&c,thumb,insn) == (valid ? ARM_OK : ARM_UNDEFINED) &&
                      c.r[15] == (valid ? 0x104u : 0x100u) && c.cycles == 1u && c.cpsr == flags && c.vfp_fpscr == fpscr,
                      "VFP vector shape T=%u D=%u op=%u len/stride=%u/%u dst=%u",thumb,dbl,op,len,stride,dst);
                bool match = true;
                for (unsigned d = 0; d < 32u; d++) match &= vfp_get_d(&c,d) == expected[d];
                CHECK(match, "VFP vector wrap/scalar/alias oracle");
            }
         }
    /* Every lane contributes flags; a later exact lane must not replace an
     * earlier overflow/invalid result or read an already published alias. */
    for (unsigned thumb = 0; thumb < 2u; thumb++) {
        arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr = ARM_FPSCR_QC | (3u<<16);
        vfp_set_d(&c,28u,UINT64_C(0x7fefffffffffffff)); vfp_set_d(&c,29u,UINT64_C(0x7ff0000000000001));
        vfp_set_d(&c,30u,UINT64_C(0x3ff0000000000000)); vfp_set_d(&c,31u,0u);
        CHECK(a8_move_step(&c,thumb,a8_fp_add(0u,1u,31u,28u,28u)) == ARM_OK &&
              vfp_get_d(&c,31u) == UINT64_C(0x7ff0000000000000) &&
              vfp_get_d(&c,28u) == UINT64_C(0x7ff8000000000001) &&
              vfp_get_d(&c,29u) == UINT64_C(0x4000000000000000) && vfp_get_d(&c,30u) == 0u &&
              c.vfp_fpscr == (ARM_FPSCR_QC | (3u<<16) | ARM_FPSCR_OFC | ARM_FPSCR_IXC | ARM_FPSCR_IOC),
              "VFP vector staged inputs/sticky flags");
        c.vfp_fpscr &= ~ARM_FPSCR_LEN;
        CHECK(a8_move_step(&c,thumb,a8_fp_add(0u,1u,20u,30u,30u)) == ARM_OK &&
              a8_move_step(&c,thumb,VMRS(2u,1u)) == ARM_OK &&
              c.r[2] == (ARM_FPSCR_QC | ARM_FPSCR_OFC | ARM_FPSCR_IXC | ARM_FPSCR_IOC),
              "VFP exact add/VMRS lost accumulated flags");
    }
}

static void test_a8_vfp_binary_access_and_invalid(void) {
    static const unsigned permissions[] = {0u,1u,3u};
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned dbl = 0; dbl < 2u; dbl++)
      for (unsigned op = 0; op < 8u; op++)
        ...
        for (unsigned enabled = 0; enabled < 2u; enabled++)
         for (unsigned access = 0; access < 3u; access++)
          for (unsigned skip = 0; skip < 2u; skip++) {
            ...
            a8_compare_value_set(&c,dbl,16u,20u); a8_fp_value_set(&c,dbl,1u,1u);
            a8_fp_value_set(&c,dbl,31u,0x12345678u);
            if (thumb) { m_w16(NULL,0x100u,skip ? 0xbf0cu : 0xbf1cu); CHECK(arm_step(&c) == ARM_OK,"VFP add IT setup"); }
            uint32_t flags = c.cpsr, fpscr = c.vfp_fpscr, pc = c.r[15];
            bool allowed = enabled && (permissions[access] == 3u || (permissions[access] == 1u && !user));
            uint32_t insn = a8_fp_binary(op,dbl,31u,16u,1u);
            if (!thumb && skip) insn &= 0x0fffffffu;
            ...
            CHECK(skip || allowed ? c.r[15] == pc+4u &&
                  ...
                  c.spsr[ARM_BANK_UND] == flags && (c.cpsr & ARM_CPSR_MODE_MASK) == ARM_MODE_UND,
                  "VFP add conditional/exception state");
          }
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned dbl = 0; dbl < 2u; dbl++)
      for (unsigned op = 0; op < 8u; op++)
       for (unsigned bit = 0; bit < 32u; bit++) {
        ...
        for (unsigned enabled = 0; enabled < 2u; enabled++)
         for (unsigned skip = 0; skip < 2u; skip++) {
            arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr |= 1u<<bit;
            c.vfp_fpexc = enabled ? ARM_FPEXC_EN : 0u; c.cp15.cpacr = enabled ? 0x00f00000u : 0u;
            for (unsigned d = 0; d < 32u; d++) vfp_set_d(&c,d,UINT64_C(0x7ff01234dead0000)+d);
            if (thumb) { m_w16(NULL,0x100u,skip ? 0xbf08u : 0xbf18u); CHECK(arm_step(&c) == ARM_OK,"invalid add IT setup"); }
            uint32_t flags = c.cpsr, fpscr = c.vfp_fpscr, pc = c.r[15];
            uint32_t insn = a8_fp_binary(op,dbl,31u,16u,1u);
            if (!thumb && skip) insn &= 0x0fffffffu;
            CHECK(a8_move_step(&c,thumb,insn) == (skip ? ARM_OK : ARM_UNDEFINED) &&
                  c.r[15] == pc+(skip ? 4u : 0u) && c.vfp_fpscr == fpscr &&
                  c.cpsr == (thumb && skip ? flags & ~0x0600fc00u : flags), "VFP reserved FPSCR/access/skip priority");
            bool match = true;
            for (unsigned d = 0; d < 32u; d++) match &= vfp_get_d(&c,d) == UINT64_C(0x7ff01234dead0000)+d;
            CHECK(match,"invalid/skipped VFP add changed FP registers");
         }
       }
    const arm_arch_t legacy[] = {ARM_ARCH_V6_ARM1176,ARM_ARCH_V7_SWIFT};
    for (unsigned profile = 0; profile < 2u; profile++)
     for (unsigned op = 0; op < 8u; op++) {
        ...
        uint32_t insn = a8_fp_binary(op,1u,16u,0u,0u);
        CHECK(a8_move_step(&c,0u,insn) == ARM_UNDEFINED &&
...
/* Independent exact tininess test: divide an all-ones integer by the second
 * significand, one bit at a time. This checks a*b < 2^k without the production
 * limb multiplication. Stop once the quotient reaches the first significand. */
static bool a8_multiply_tiny(uint64_t a, uint64_t b, unsigned dbl) {
    unsigned fraction = dbl ? 52u : 23u;
    uint64_t hidden = UINT64_C(1) << fraction;
    uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
    unsigned ea = (unsigned)((a & ~sign) >> fraction), eb = (unsigned)((b & ~sign) >> fraction);
    uint64_t sa = (a & (hidden-1u)) + (ea ? hidden : 0u), sb = (b & (hidden-1u)) + (eb ? hidden : 0u);
    if (!sa || !sb) return false;
    int power = 1 + (dbl ? 1023 : 127) + 2*(int)fraction - (int)(ea ? ea : 1u) - (int)(eb ? eb : 1u);
    if (power <= 0) return false;
    if (power >= 2*(int)fraction+2) return true;
    uint64_t quotient = 0u, remainder = 0u;
    for (int bit = 0; bit < power; bit++) {
        remainder = remainder*2u+1u; quotient *= 2u;
        if (remainder >= sb) { remainder -= sb; quotient++; }
        if (quotient >= sa) return true;
    }
    return false;
}

static uint64_t a8_multiply_native(uint64_t a, uint64_t b, unsigned dbl, unsigned neg,
                                   uint32_t fpscr, uint32_t *flags) {
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
    uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
    uint64_t normal = dbl ? UINT64_C(0x0010000000000000) : UINT64_C(0x00800000);
    *flags = 0u;
    ...
    bool tiny = a8_multiply_tiny(a,b,dbl);
    fenv_t saved;
    CHECK(fegetenv(&saved) == 0 && fesetround(rounds[(fpscr>>22)&3u]) == 0 &&
          feclearexcept(FE_ALL_EXCEPT) == 0,"prepare native multiply oracle");
    uint64_t bits;
    if (dbl) {
        volatile double x = u2d(a), y = u2d(b);
        volatile double z = x*y; bits = d2u(z);
    } else {
        volatile float x = u2f((uint32_t)a), y = u2f((uint32_t)b);
        volatile float z = x*y; bits = f2u(z);
    }
    int exceptions = fetestexcept(FE_ALL_EXCEPT);
    CHECK(!(exceptions & (FE_INVALID | FE_DIVBYZERO)),"nonfinite input reached finite multiply oracle");
    if (tiny && (fpscr & ARM_FPSCR_FZ)) { bits = (a ^ b) & sign; *flags |= ARM_FPSCR_UFC; }
    else {
        if (exceptions & FE_OVERFLOW) *flags |= ARM_FPSCR_OFC;
        if (exceptions & FE_INEXACT) { *flags |= ARM_FPSCR_IXC; if (tiny) *flags |= ARM_FPSCR_UFC; }
    }
    /* ARM detects tininess before rounding. Native FE_UNDERFLOW can instead
...
static void test_a8_vfp_multiply_registers(void) {
    CHECK(a8_fp_multiply(0u,1u,20u,16u,18u) == 0xee604ba2u &&
          a8_fp_multiply(1u,0u,31u,16u,17u) == 0xee68fa68u,"VFP multiply encoding anchors");
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned dbl = 0; dbl < 2u; dbl++)
       ...
       for (unsigned dst = 0; dst < 32u; dst++)
        for (unsigned left = 0; left < 32u; left++)
         for (unsigned right = 0; right < 32u; right++) {
            ...
            for (unsigned r = 0; r < 32u; r++) a8_fp_value_set(&c,dbl,r,a8_add_integer(dbl,(int)r-16));
            uint64_t expected[32]; for (unsigned d = 0; d < 32u; d++) expected[d] = vfp_get_d(&c,d);
            int x = (int)left-16, y = (int)right-16, product = x*y;
            uint64_t want = a8_add_integer(dbl,neg ? -product : product);
            if (!product && ((x < 0) != (y < 0)) != (neg != 0u))
                want = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
            a8_add_expected_set(expected,dbl,dst,want);
            c.excl_valid = true; c.excl_addr = 0x12340u;
            uint32_t flags = c.cpsr, fpscr = c.vfp_fpscr;
            CHECK(a8_move_step(&c,thumb,a8_fp_multiply(neg,dbl,dst,left,right)) == ARM_OK &&
                  c.r[15] == 0x104u && c.cycles == 1u && c.cpsr == flags && c.vfp_fpscr == fpscr &&
                  c.excl_valid && c.excl_addr == 0x12340u,"VFP multiply register state T=%u D=%u neg=%u d/n/m=%u/%u/%u",
                  thumb,dbl,neg,dst,left,right);
            bool match = true;
            for (unsigned d = 0; d < 32u; d++) match &= vfp_get_d(&c,d) == expected[d];
            for (unsigned r = 0; r < 15u; r++) match &= c.r[r] == 0u;
            CHECK(match,"VFP multiply register/alias/signed-zero oracle");
         }
}

static void test_a8_vfp_multiply_values_and_host_state(void) {
    fenv_t saved; CHECK(fegetenv(&saved) == 0,"save VFP multiply host state");
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
    for (unsigned dbl = 0; dbl < 2u; dbl++) {
        uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
        uint64_t normal = dbl ? UINT64_C(0x0010000000000000) : UINT64_C(0x00800000);
        uint64_t infinity = dbl ? UINT64_C(0x7ff0000000000000) : UINT64_C(0x7f800000);
        uint64_t one = dbl ? UINT64_C(0x3ff0000000000000) : UINT64_C(0x3f800000);
        uint64_t half = dbl ? UINT64_C(0x3fe0000000000000) : UINT64_C(0x3f000000);
        uint64_t two = dbl ? UINT64_C(0x4000000000000000) : UINT64_C(0x40000000);
        const struct { uint64_t a,b,result[4]; uint32_t flags; bool tiny; } anchors[] = {
            {one,one,{one,one,one,one},0u,false},
            {sign,one,{sign,sign,sign,sign},0u,false},
            {one+1u,one+1u,{one+2u,one+3u,one+2u,one+2u},ARM_FPSCR_IXC,false},
            {one-1u,one+1u,{one,one+1u,one,one},ARM_FPSCR_IXC,false},
            {normal,half,{normal/2u,normal/2u,normal/2u,normal/2u},0u,true},
            {1u,half,{0u,1u,0u,0u},ARM_FPSCR_UFC|ARM_FPSCR_IXC,true},
            {normal,one-1u,{normal,normal,normal-1u,normal-1u},ARM_FPSCR_UFC|ARM_FPSCR_IXC,true},
            {normal,one-2u,{normal-1u,normal-1u,normal-1u,normal-1u},0u,true},
            {infinity-1u,two,{infinity,infinity,infinity-1u,infinity-1u},ARM_FPSCR_OFC|ARM_FPSCR_IXC,false}
        };
        for (unsigned row = 0; row < sizeof anchors / sizeof anchors[0]; row++)
         for (unsigned mode = 0; mode < 4u; mode++)
          for (unsigned neg = 0; neg < 2u; neg++) {
            uint32_t exceptions;
            uint64_t want = anchors[row].result[mode] ^ (neg ? sign : 0u);
            CHECK(a8_multiply_tiny(anchors[row].a,anchors[row].b,dbl) == anchors[row].tiny,"multiply tininess anchor");
            CHECK(a8_multiply_native(anchors[row].a,anchors[row].b,dbl,neg,mode<<22,&exceptions) == want &&
                  exceptions == anchors[row].flags,"native multiply anchor D=%u row=%u mode=%u neg=%u",dbl,row,mode,neg);
            for (unsigned thumb = 0; thumb < 2u; thumb++) {
                arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr = ARM_FPSCR_QC | ARM_FPSCR_DZC | (mode<<22);
                a8_fp_value_set(&c,dbl,16u,anchors[row].a); a8_fp_value_set(&c,dbl,1u,anchors[row].b);
                uint32_t flags = c.vfp_fpscr;
                CHECK(a8_move_step(&c,thumb,a8_fp_multiply(neg,dbl,31u,16u,1u)) == ARM_OK &&
                      a8_fp_value(&c,dbl,31u) == want && c.vfp_fpscr == (flags | anchors[row].flags),"VFP multiply analytical anchor");
            }
          }
        uint64_t random = UINT64_C(0x91a2b3c4d5e6f708);
        for (unsigned sample = 0; sample < 1024u; sample++) {
            random ^= random << 13; random ^= random >> 7; random ^= random << 17;
            uint64_t a = dbl ? random : (uint32_t)random;
            random ^= random << 13; random ^= random >> 7; random ^= random << 17;
            ...
            if ((a & ~sign) >= infinity) a &= ~infinity;
            if ((b & ~sign) >= infinity) b &= ~infinity;
            if (sample % 8u == 0u) { a = normal; b = one-1u; }
            if (sample % 8u == 1u) a &= sign | (normal-1u);
            if (sample % 8u == 2u) { a = normal; b = half; }
            if (sample % 8u == 3u) { a = 1u; b = half; }
            if (sample & 8u) a ^= sign;
            for (unsigned controls = 0; controls < 16u; controls++)
             for (unsigned neg = 0; neg < 2u; neg++) {
                uint32_t fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC | ARM_FPSCR_DZC |
                    ((controls&3u)<<22) | (controls&4u ? ARM_FPSCR_FZ : 0u) | (controls&8u ? ARM_FPSCR_DN : 0u), exceptions;
                uint64_t want = a8_multiply_native(a,b,dbl,neg,fpscr,&exceptions);
                for (unsigned thumb = 0; thumb < 2u; thumb++) {
                    arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr = fpscr;
                    a8_fp_value_set(&c,dbl,16u,a); a8_fp_value_set(&c,dbl,1u,b);
                    unsigned host = sample % 4u;
                    CHECK(fesetround(rounds[host]) == 0 && feclearexcept(FE_ALL_EXCEPT) == 0 &&
                          (!(sample&1u) || feraiseexcept(FE_INVALID | FE_DIVBYZERO) == 0),"prepare multiply host state");
                    int pending = fetestexcept(FE_ALL_EXCEPT); uint32_t flags = c.cpsr;
                    CHECK(a8_move_step(&c,thumb,a8_fp_multiply(neg,dbl,31u,16u,1u)) == ARM_OK &&
                          a8_fp_value(&c,dbl,31u) == want && c.vfp_fpscr == (fpscr | exceptions) && c.cpsr == flags,
                          "VFP finite multiply D=%u T=%u neg=%u controls=%u sample=%u",dbl,thumb,neg,controls,sample);
                    CHECK(fegetround() == rounds[host] && fetestexcept(FE_ALL_EXCEPT) == pending,"VFP multiply changed host FP state");
                }
             }
        }
    }
    CHECK(fesetenv(&saved) == 0,"restore VFP multiply host state");
}

static void test_a8_vfp_multiply_special_values(void) {
    fenv_t saved; CHECK(fegetenv(&saved) == 0,"save special multiply host state");
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
    unsigned count = sizeof a8_compare_values / sizeof a8_compare_values[0];
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned dbl = 0; dbl < 2u; dbl++)
       ...
       for (unsigned controls = 0; controls < 16u; controls++)
        for (unsigned x = 0; x < count; x++)
         for (unsigned y = 0; y < count; y++) {
            bool nan = a8_compare_values[x].nan || a8_compare_values[y].nan;
            bool inf_x = x == 0u || x == 17u, inf_y = y == 0u || y == 17u;
            bool zero_x = x == 8u || x == 9u || ((controls&4u) && a8_compare_values[x].denormal);
            bool zero_y = y == 8u || y == 9u || ((controls&4u) && a8_compare_values[y].denormal);
            if (!nan && !inf_x && !inf_y && !zero_x && !zero_y) continue;
            uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
            uint64_t infinity = dbl ? UINT64_C(0x7ff0000000000000) : UINT64_C(0x7f800000);
            uint64_t quiet = dbl ? UINT64_C(0x0008000000000000) : UINT64_C(0x00400000);
            uint64_t a = dbl ? a8_compare_values[x].dual : a8_compare_values[x].single;
            ...
            if (nan) {
                ...
                want = controls&8u ? infinity | quiet :
                    (dbl ? a8_compare_values[chosen].dual : a8_compare_values[chosen].single) | quiet;
                if (a8_compare_values[x].signaling || a8_compare_values[y].signaling) exceptions |= ARM_FPSCR_IOC;
            } else if ((inf_x && zero_y) || (inf_y && zero_x)) {
                want = infinity | quiet; exceptions |= ARM_FPSCR_IOC;
            } else want = ((a ^ b) & sign) | (inf_x || inf_y ? infinity : 0u);
            if (neg) want ^= sign;
            arm_cpu_t c; a8_move_reset(&c,thumb);
            c.vfp_fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC | ARM_FPSCR_DZC |
                ((controls&3u)<<22) | (controls&4u ? ARM_FPSCR_FZ : 0u) | (controls&8u ? ARM_FPSCR_DN : 0u);
            a8_fp_value_set(&c,dbl,16u,a); a8_fp_value_set(&c,dbl,1u,b);
            uint32_t flags = c.vfp_fpscr; unsigned host = (x+y)%4u;
            CHECK(fesetround(rounds[host]) == 0 && feclearexcept(FE_ALL_EXCEPT) == 0 &&
                  (!((x+y)&1u) || feraiseexcept(FE_DIVBYZERO) == 0),"prepare special multiply host state");
            int pending = fetestexcept(FE_ALL_EXCEPT);
            CHECK(a8_move_step(&c,thumb,a8_fp_multiply(neg,dbl,31u,16u,1u)) == ARM_OK &&
                  a8_fp_value(&c,dbl,31u) == want && c.vfp_fpscr == (flags | exceptions),
                  "VFP special multiply D=%u T=%u neg=%u controls=%u x/y=%u/%u",dbl,thumb,neg,controls,x,y);
            CHECK(fegetround() == rounds[host] && fetestexcept(FE_ALL_EXCEPT) == pending,"special multiply changed host FP state");
         }
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned neg = 0; neg < 2u; neg++) {
        arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr = ARM_FPSCR_QC | ARM_FPSCR_FZ | (3u<<16);
        uint64_t flip = neg ? UINT64_C(0x8000000000000000) : 0u;
        vfp_set_d(&c,28u,UINT64_C(0x7fefffffffffffff)); vfp_set_d(&c,29u,UINT64_C(0x0010000000000000));
        vfp_set_d(&c,30u,UINT64_C(0x7ff0000000000001)); vfp_set_d(&c,31u,UINT64_C(0x3ff0000000000000));
        uint32_t flags = c.vfp_fpscr | ARM_FPSCR_OFC | ARM_FPSCR_IXC | ARM_FPSCR_UFC | ARM_FPSCR_IOC;
        CHECK(a8_move_step(&c,thumb,a8_fp_multiply(neg,1u,31u,28u,28u)) == ARM_OK &&
              vfp_get_d(&c,31u) == (UINT64_C(0x7ff0000000000000) ^ flip) && vfp_get_d(&c,28u) == flip &&
              vfp_get_d(&c,29u) == (UINT64_C(0x7ff8000000000001) ^ flip) &&
              vfp_get_d(&c,30u) == (UINT64_C(0x3ff0000000000000) ^ flip) && c.vfp_fpscr == flags,
              "VFP multiply vector staged inputs/sticky flags");
        c.vfp_fpscr &= ~ARM_FPSCR_LEN;
        CHECK(a8_move_step(&c,thumb,a8_fp_multiply(0u,1u,20u,30u,30u)) == ARM_OK &&
              a8_move_step(&c,thumb,VMRS(2u,1u)) == ARM_OK && c.r[2] == (flags & ~ARM_FPSCR_LEN),
              "VFP exact multiply/VMRS lost accumulated flags");
     }
    CHECK(fesetenv(&saved) == 0,"restore special multiply host state");
}

/* Exact ratio threshold, independent of the production quotient algorithm:
 * sa/sb * 2^(ea-eb) < 2^(1-bias). Integer shifts and comparisons avoid both
 * floating-point underflow and any need to form an oversized product. */
static bool a8_divide_tiny(uint64_t a, uint64_t b, unsigned dbl) {
    unsigned fraction = dbl ? 52u : 23u;
    uint64_t hidden = UINT64_C(1) << fraction;
    uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
    unsigned ea = (unsigned)((a & ~sign) >> fraction), eb = (unsigned)((b & ~sign) >> fraction);
    uint64_t sa = (a & (hidden-1u)) + (ea ? hidden : 0u), sb = (b & (hidden-1u)) + (eb ? hidden : 0u);
    if (!sa || !sb) return false;
    int shift = (int)(ea ? ea : 1u) - (int)(eb ? eb : 1u) + (dbl ? 1023 : 127) - 1;
    if (shift >= 0) return shift < 64 && sa <= ((sb-1u) >> (unsigned)shift);
    return shift <= -64 || (sa >> (unsigned)-shift) < sb;
}

static uint64_t a8_divide_native(uint64_t a, uint64_t b, unsigned dbl, uint32_t fpscr, uint32_t *flags) {
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
    uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
    uint64_t normal = dbl ? UINT64_C(0x0010000000000000) : UINT64_C(0x00800000);
    uint64_t infinity = dbl ? UINT64_C(0x7ff0000000000000) : UINT64_C(0x7f800000);
    *flags = 0u;
    CHECK((a & ~sign) < infinity && (b & ~sign) < infinity,"nonfinite input reached finite divide oracle");
    if (fpscr & ARM_FPSCR_FZ) {
        if ((a & ~sign) && (a & ~sign) < normal) { a &= sign; *flags |= ARM_FPSCR_IDC; }
        if ((b & ~sign) && (b & ~sign) < normal) { b &= sign; *flags |= ARM_FPSCR_IDC; }
    }
    if (!(b & ~sign)) {
        if (!(a & ~sign)) { *flags |= ARM_FPSCR_IOC; return infinity | (normal >> 1); }
        *flags |= ARM_FPSCR_DZC; return ((a ^ b) & sign) | infinity;
    }
    bool tiny = a8_divide_tiny(a,b,dbl);
    fenv_t saved;
    CHECK(fegetenv(&saved) == 0 && fesetround(rounds[(fpscr>>22)&3u]) == 0 &&
          feclearexcept(FE_ALL_EXCEPT) == 0,"prepare native divide oracle");
    uint64_t bits;
    if (dbl) {
        volatile double x = u2d(a), y = u2d(b);
        volatile double z = x/y; bits = d2u(z);
    ...
    int exceptions = fetestexcept(FE_ALL_EXCEPT);
    CHECK(!(exceptions & (FE_INVALID | FE_DIVBYZERO)),"unexpected native divide exception");
    if (tiny && (fpscr & ARM_FPSCR_FZ)) { bits = (a ^ b) & sign; *flags |= ARM_FPSCR_UFC; }
    else {
        if (exceptions & FE_OVERFLOW) *flags |= ARM_FPSCR_OFC;
        if (exceptions & FE_INEXACT) { *flags |= ARM_FPSCR_IXC; if (tiny) *flags |= ARM_FPSCR_UFC; }
    }
    /* Host FE_UNDERFLOW may detect tininess after rounding; the exact ratio
     * comparison above supplies ARM's before-rounding rule instead. */
    CHECK(fesetenv(&saved) == 0,"restore native divide oracle host state");
    return bits;
}

static void test_a8_vfp_divide_registers(void) {
    CHECK(a8_fp_divide(1u,16u,17u,16u) == 0xeec10ba0u &&
          a8_fp_divide(0u,31u,16u,17u) == 0xeec8fa28u,"VFP divide encoding anchors");
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned dbl = 0; dbl < 2u; dbl++)
      for (unsigned dst = 0; dst < 32u; dst++)
       for (unsigned left = 0; left < 32u; left++)
        for (unsigned right = 0; right < 32u; right++) {
            ...
            c.vfp_fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC | ARM_FPSCR_DZC;
            for (unsigned d = 0; d < 32u; d++) vfp_set_d(&c,d,UINT64_C(0x7ff01234dead0000)+d);
            for (unsigned r = 0; r < 32u; r++) a8_fp_value_set(&c,dbl,r,a8_power_of_two(dbl,(int)r-16,r&1u));
            uint64_t expected[32]; for (unsigned d = 0; d < 32u; d++) expected[d] = vfp_get_d(&c,d);
            a8_add_expected_set(expected,dbl,dst,a8_power_of_two(dbl,(int)left-(int)right,(left^right)&1u));
            c.excl_valid = true; c.excl_addr = 0x12340u;
            uint32_t flags = c.cpsr, fpscr = c.vfp_fpscr;
            CHECK(a8_move_step(&c,thumb,a8_fp_divide(dbl,dst,left,right)) == ARM_OK &&
                  c.r[15] == 0x104u && c.cycles == 1u && c.cpsr == flags && c.vfp_fpscr == fpscr &&
                  c.excl_valid && c.excl_addr == 0x12340u,"VFP divide register state T=%u D=%u d/n/m=%u/%u/%u",
                  thumb,dbl,dst,left,right);
            bool match = true;
            for (unsigned d = 0; d < 32u; d++) match &= vfp_get_d(&c,d) == expected[d];
            for (unsigned r = 0; r < 15u; r++) match &= c.r[r] == 0u;
            CHECK(match,"VFP divide register/alias/sign oracle");
        }
}

static void test_a8_vfp_divide_values_and_host_state(void) {
    fenv_t saved; CHECK(fegetenv(&saved) == 0,"save VFP divide host state");
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
    for (unsigned dbl = 0; dbl < 2u; dbl++) {
        uint64_t sign = dbl ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
        uint64_t normal = dbl ? UINT64_C(0x0010000000000000) : UINT64_C(0x00800000);
        uint64_t infinity = dbl ? UINT64_C(0x7ff0000000000000) : UINT64_C(0x7f800000);
        uint64_t one = a8_power_of_two(dbl,0,0u), two = a8_power_of_two(dbl,1,0u);
        uint64_t half = a8_power_of_two(dbl,-1,0u);
        uint64_t three = dbl ? UINT64_C(0x4008000000000000) : UINT64_C(0x40400000);
        uint64_t third_low = dbl ? UINT64_C(0x3fd5555555555555) : UINT64_C(0x3eaaaaaa);
        const struct { uint64_t a,b,result[4]; uint32_t flags; bool tiny; } anchors[] = {
            {one,one,{one,one,one,one},0u,false},
            {0u,one,{0u,0u,0u,0u},0u,false},
            {one,three,{third_low+(dbl ? 0u : 1u),third_low+1u,third_low,third_low},ARM_FPSCR_IXC,false},
            {one+1u,one,{one+1u,one+1u,one+1u,one+1u},0u,false},
            {normal,two,{normal/2u,normal/2u,normal/2u,normal/2u},0u,true},
            {1u,two,{0u,1u,0u,0u},ARM_FPSCR_UFC|ARM_FPSCR_IXC,true},
            {normal,one+1u,{normal-1u,normal,normal-1u,normal-1u},ARM_FPSCR_UFC|ARM_FPSCR_IXC,true},
            {normal,one-1u,{normal+1u,normal+1u,normal,normal},ARM_FPSCR_IXC,false},
            {1u,one,{1u,1u,1u,1u},0u,true},
            {infinity-1u,half,{infinity,infinity,infinity-1u,infinity-1u},ARM_FPSCR_OFC|ARM_FPSCR_IXC,false}
        };
        for (unsigned row = 0; row < sizeof anchors / sizeof anchors[0]; row++)
         for (unsigned mode = 0; mode < 4u; mode++)
          for (unsigned neg = 0; neg < 2u; neg++)
           for (unsigned fz = 0; fz < 2u; fz++) {
            uint64_t a = anchors[row].a ^ (neg ? sign : 0u), b = anchors[row].b;
            unsigned effective_mode = neg && (mode == 1u || mode == 2u) ? 3u-mode : mode;
            uint64_t want = anchors[row].result[effective_mode] ^ (neg ? sign : 0u);
            uint32_t exceptions, want_flags = anchors[row].flags, controls = (mode<<22) | (fz ? ARM_FPSCR_FZ : 0u);
            if (fz && anchors[row].a && anchors[row].a < normal) { want = a & sign; want_flags = ARM_FPSCR_IDC; }
            else if (fz && anchors[row].tiny) { want = a & sign; want_flags = ARM_FPSCR_UFC; }
            CHECK(a8_divide_tiny(a,b,dbl) == anchors[row].tiny,"divide tininess anchor");
            CHECK(a8_divide_native(a,b,dbl,controls,&exceptions) == want && exceptions == want_flags,
                  "native divide anchor D=%u row=%u mode=%u neg=%u FZ=%u",dbl,row,mode,neg,fz);
            for (unsigned thumb = 0; thumb < 2u; thumb++) {
                arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr = ARM_FPSCR_QC | controls;
                a8_fp_value_set(&c,dbl,16u,a); a8_fp_value_set(&c,dbl,1u,b);
                CHECK(a8_move_step(&c,thumb,a8_fp_divide(dbl,31u,16u,1u)) == ARM_OK &&
                      a8_fp_value(&c,dbl,31u) == want && c.vfp_fpscr == (ARM_FPSCR_QC | controls | want_flags),
                      "VFP divide analytical anchor D=%u row=%u mode=%u neg=%u FZ=%u T=%u",dbl,row,mode,neg,fz,thumb);
            }
           }
        uint64_t random = UINT64_C(0x8129a3b4c5d6e7f0);
        for (unsigned sample = 0; sample < 1024u; sample++) {
            random ^= random << 13; random ^= random >> 7; random ^= random << 17;
            uint64_t a = dbl ? random : (uint32_t)random;
            random ^= random << 13; random ^= random >> 7; random ^= random << 17;
            ...
            if (sample % 8u == 0u) { a = normal; b = one+1u; }
            if (sample % 8u == 1u) a &= sign | (normal-1u);
            if (sample % 8u == 2u) { a = normal; b = two; }
            if (sample % 8u == 3u) { a = 1u; b = two; }
            if (sample % 8u == 4u) b &= sign | (normal-1u);
            if (sample & 8u) a ^= sign;
            for (unsigned controls = 0; controls < 16u; controls++) {
                uint32_t fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC |
                    ((controls&3u)<<22) | (controls&4u ? ARM_FPSCR_FZ : 0u) | (controls&8u ? ARM_FPSCR_DN : 0u), exceptions;
                uint64_t want = a8_divide_native(a,b,dbl,fpscr,&exceptions);
                for (unsigned thumb = 0; thumb < 2u; thumb++) {
                    arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr = fpscr;
                    a8_fp_value_set(&c,dbl,16u,a); a8_fp_value_set(&c,dbl,1u,b);
                    unsigned host = sample % 4u;
                    CHECK(fesetround(rounds[host]) == 0 && feclearexcept(FE_ALL_EXCEPT) == 0 &&
                          (!(sample&1u) || feraiseexcept(FE_INVALID | FE_DIVBYZERO) == 0),"prepare divide host state");
                    int pending = fetestexcept(FE_ALL_EXCEPT); uint32_t flags = c.cpsr;
                    CHECK(a8_move_step(&c,thumb,a8_fp_divide(dbl,31u,16u,1u)) == ARM_OK &&
                          a8_fp_value(&c,dbl,31u) == want && c.vfp_fpscr == (fpscr | exceptions) && c.cpsr == flags,
                          "VFP finite divide D=%u T=%u controls=%u sample=%u",dbl,thumb,controls,sample);
                    CHECK(fegetround() == rounds[host] && fetestexcept(FE_ALL_EXCEPT) == pending,"VFP divide changed host FP state");
                }
            }
        }
    }
    CHECK(fesetenv(&saved) == 0,"restore VFP divide host state");
}

static void test_a8_vfp_divide_special_values(void) {
    fenv_t saved; CHECK(fegetenv(&saved) == 0,"save special divide host state");
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
    unsigned count = sizeof a8_compare_values / sizeof a8_compare_values[0];
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned dbl = 0; dbl < 2u; dbl++)
        ...
        for (unsigned y = 0; y < count; y++) {
            ...
            if (nan) {
                ...
                want = controls&8u ? infinity | quiet :
                    (dbl ? a8_compare_values[chosen].dual : a8_compare_values[chosen].single) | quiet;
                if (a8_compare_values[x].signaling || a8_compare_values[y].signaling) exceptions |= ARM_FPSCR_IOC;
            } else if ((inf_x && inf_y) || (zero_x && zero_y)) {
                want = infinity | quiet; exceptions |= ARM_FPSCR_IOC;
            } else {
                want = ((a ^ b) & sign) | (inf_x || zero_y ? infinity : 0u);
                if (zero_y && !inf_x) exceptions |= ARM_FPSCR_DZC;
            }
            arm_cpu_t c; a8_move_reset(&c,thumb);
            c.vfp_fpscr = ARM_FPSCR_NZCV | ARM_FPSCR_QC |
                ((controls&3u)<<22) | (controls&4u ? ARM_FPSCR_FZ : 0u) | (controls&8u ? ARM_FPSCR_DN : 0u);
            a8_fp_value_set(&c,dbl,16u,a); a8_fp_value_set(&c,dbl,1u,b);
            uint32_t flags = c.vfp_fpscr; unsigned host = (x+y)%4u;
            CHECK(fesetround(rounds[host]) == 0 && feclearexcept(FE_ALL_EXCEPT) == 0 &&
                  (!((x+y)&1u) || feraiseexcept(FE_DIVBYZERO) == 0),"prepare special divide host state");
            int pending = fetestexcept(FE_ALL_EXCEPT);
            CHECK(a8_move_step(&c,thumb,a8_fp_divide(dbl,31u,16u,1u)) == ARM_OK &&
                  a8_fp_value(&c,dbl,31u) == want && c.vfp_fpscr == (flags | exceptions),
                  "VFP special divide D=%u T=%u controls=%u x/y=%u/%u",dbl,thumb,controls,x,y);
            CHECK(fegetround() == rounds[host] && fetestexcept(FE_ALL_EXCEPT) == pending,"special divide changed host FP state");
        }
    /* Circular destination aliases the numerator bank. Later lanes must read
     * originals and retain earlier divide-by-zero/invalid/inexact flags. */
    for (unsigned thumb = 0; thumb < 2u; thumb++) {
        arm_cpu_t c; a8_move_reset(&c,thumb); c.vfp_fpscr = ARM_FPSCR_QC | (3u<<16);
        vfp_set_d(&c,28u,UINT64_C(0x3ff0000000000000)); vfp_set_d(&c,29u,0u);
        vfp_set_d(&c,30u,UINT64_C(0x3ff0000000000000)); vfp_set_d(&c,31u,UINT64_C(0x4018000000000000));
        vfp_set_d(&c,24u,0u); vfp_set_d(&c,25u,0u);
        vfp_set_d(&c,26u,UINT64_C(0x4008000000000000)); vfp_set_d(&c,27u,UINT64_C(0x4000000000000000));
        uint32_t flags = c.vfp_fpscr | ARM_FPSCR_DZC | ARM_FPSCR_IOC | ARM_FPSCR_IXC;
        CHECK(a8_move_step(&c,thumb,a8_fp_divide(1u,31u,28u,24u)) == ARM_OK &&
              vfp_get_d(&c,31u) == UINT64_C(0x7ff0000000000000) && vfp_get_d(&c,28u) == UINT64_C(0x7ff8000000000000) &&
              vfp_get_d(&c,29u) == UINT64_C(0x3fd5555555555555) && vfp_get_d(&c,30u) == UINT64_C(0x4008000000000000) &&
              c.vfp_fpscr == flags,"VFP divide vector staged inputs/sticky flags");
        c.vfp_fpscr &= ~ARM_FPSCR_LEN;
        CHECK(a8_move_step(&c,thumb,a8_fp_divide(1u,20u,30u,30u)) == ARM_OK &&
              vfp_get_d(&c,20u) == UINT64_C(0x3ff0000000000000) &&
              a8_move_step(&c,thumb,VMRS(2u,1u)) == ARM_OK && c.r[2] == (flags & ~ARM_FPSCR_LEN),
...
static uint32_t a8_neon_lane(unsigned thumb,unsigned load_,unsigned size,unsigned index_align,unsigned d,unsigned rn,unsigned rm) {
    return (thumb ? 0xf9800000u : 0xf4800000u)|(load_<<21)|((d&15u)<<12)|((d>>4)<<22)|
...
static void test_a8_neon_lane_registers(void) {
    CHECK(a8_neon_lane(0u,1u,2u,0u,6u,0u,13u)==0xf4a0680du &&
          ...
          a8_neon_lane(1u,1u,2u,11u,31u,13u,14u)==0xf9edf8beu &&
          a8_neon_lane(0u,0u,2u,0u,0u,10u,13u)==0xf48a080du &&
          a8_neon_lane(1u,0u,2u,11u,31u,13u,14u)==0xf9cdf8beu,"lane transfer encoding anchors");
    for (unsigned thumb=0;thumb<2u;thumb++) for (unsigned load_=0;load_<2u;load_++) for (unsigned d=0;d<32u;d++)
     for (unsigned lane=0;lane<2u;lane++) for (unsigned align=0;align<2u;align++)
      for (unsigned rn=0;rn<15u;rn++) for (unsigned rm=0;rm<16u;rm++) {
        ...
        memset(g_ram+0x2030u,0xa5,32u); m_w32(NULL,0x2040u,load_ ? raw : 0x13579bdfu);
        ...
        if (!load_) { vfp_set_d(&c,d,expected_fp[d]); memcpy(expected_ram+16u,&raw,4u); }
        if (rm!=15u) expected_gpr[rn]+=rm==13u ? 4u : c.r[rm];
        uint32_t flags=c.cpsr;
        CHECK(a8_move_step(&c,thumb,a8_neon_lane(thumb,load_,2u,lane*8u+align*3u,d,rn,rm))==ARM_OK &&
              c.r[15]==0x104u && c.cycles==1u && c.cpsr==flags && c.vfp_fpscr==0xfff79f9fu &&
              c.vfp_fpexc==ARM_FPEXC_EN && c.excl_valid && c.excl_addr==0x2468u && c.a8_excl_size==8u,"lane transfer status");
        ...
        CHECK(match,"lane registers/bytes/writeback T=%u L=%u D=%u lane=%u align=%u Rn/m=%u/%u",thumb,load_,d,lane,align,rn,rm);
      }
}

static void test_a8_neon_lane_alignment_and_access(void) {
    static const unsigned permissions[]={0u,1u,3u};
    for (unsigned thumb=0;thumb<2u;thumb++) for (unsigned load_=0;load_<2u;load_++) for (unsigned lane=0;lane<2u;lane++)
     for (unsigned align=0;align<2u;align++) for (unsigned alignment_check=0;alignment_check<2u;alignment_check++)
      for (unsigned offset=0;offset<4u;offset++) for (unsigned enabled=0;enabled<2u;enabled++)
       for (unsigned access=0;access<3u;access++) for (unsigned user=0;user<2u;user++) {
        ...
        CHECK(a8_move_step(&c,thumb,a8_neon_lane(thumb,load_,2u,lane*8u+align*3u,31u,5u,13u))==
        ...
        if (!allowed) CHECK(c.r[15]==ARM_VEC_UNDEFINED && c.r[14]==(thumb ? 0x102u : 0x104u) &&
            ...
            c.cp15.dfar==0x2040u+offset && c.cp15.dfsr==(1u|(load_ ? 0u : 1u<<11)),"lane precise alignment abort");
        ...
        if (executed && load_) expected=lane ? UINT64_C(0x7f8000019abcdef0) : UINT64_C(0x123456787f800001);
        uint32_t stored=executed && !load_ ? (lane ? 0x12345678u : 0x9abcdef0u) : 0x7f800001u;
        CHECK(vfp_get_d(&c,31u)==expected && m_r32(NULL,0x2040u)==stored,"lane alignment/access changed uncompleted data");
       }
}

static void test_a8_neon_lane_invalid_and_it(void) {
    for (unsigned thumb=0;thumb<2u;thumb++) for (unsigned load_=0;load_<2u;load_++) for (unsigned size=0;size<4u;size++)
     for (unsigned index_align=0;index_align<16u;index_align++) for (unsigned bad_base=0;bad_base<2u;bad_base++)
      for (unsigned big=0;big<2u;big++) for (unsigned enabled=0;enabled<2u;enabled++)
       for (unsigned skip=0;skip<(thumb ? 2u : 1u);skip++) {
        ...
        uint32_t insn=a8_neon_lane(thumb,load_,size,index_align,31u,bad_base ? 15u : 5u,13u);
        ...
        uint64_t expected=!executed || !load_ ? UINT64_C(0x123456789abcdef0) : index_align&8u ?
            UINT64_C(0x7f8000019abcdef0) : UINT64_C(0x123456787f800001);
        uint32_t stored=executed && !load_ ? (index_align&8u ? 0x12345678u : 0x9abcdef0u) : 0x7f800001u;
        CHECK(vfp_get_d(&c,31u)==expected && m_r32(NULL,0x2040u)==stored,"lane refused or skipped result");
       }
    const arm_arch_t legacy[]={ARM_ARCH_V6_ARM1176,ARM_ARCH_V7_SWIFT};
    for (unsigned profile=0;profile<2u;profile++) for (unsigned load_=0;load_<2u;load_++) for (unsigned hint=0;hint<2u;hint++) {
        ...
        CHECK(a8_move_step(&c,0u,a8_neon_lane(0u,load_,2u,0u,hint ? 31u : 16u,5u,13u))==
              (hint ? ARM_OK : ARM_UNDEFINED) && c.r[15]==(hint ? 4u : 0u) &&
              c.r[5]==0x2040u,"lane load changed legacy disposition");
    }
    static const uint32_t toggles[]={1u<<8,1u<<9,3u<<8};
    for (unsigned thumb=0;thumb<2u;thumb++) for (unsigned load_=0;load_<2u;load_++) for (unsigned enabled=0;enabled<2u;enabled++)
     for (unsigned neighbor=0;neighbor<3u;neighbor++) {
        arm_cpu_t c; a8_move_reset(&c,thumb); c.r[5]=0x2040u; c.vfp_fpexc=enabled ? ARM_FPEXC_EN : 0u;
        uint32_t flags=c.cpsr,fpscr=c.vfp_fpscr;
        CHECK(a8_move_step(&c,thumb,a8_neon_lane(thumb,load_,2u,0u,31u,5u,13u)^toggles[neighbor])==ARM_UNDEFINED &&
              c.r[15]==0x100u && c.r[5]==0x2040u && c.cpsr==flags && c.vfp_fpscr==fpscr,"lane transfer claimed neighboring structure");
     }
}
...
static void a8_transpose_expected(uint64_t values[32], unsigned size, unsigned quad, unsigned d, unsigned m) {
    /* Independent byte-array oracle: even output elements come from the
     * first input's even/odd elements; odd ones from the second input. */
    uint8_t a[16], b[16];
    unsigned bytes = (quad + 1u) * 8u, element_bytes = 1u << size;
    for (unsigned i = 0; i < bytes; i++) {
        a[i] = (uint8_t)(values[d + i / 8u] >> ((i % 8u) * 8u));
        b[i] = (uint8_t)(values[m + i / 8u] >> ((i % 8u) * 8u));
    }
    for (unsigned r = 0; r <= quad; r++) values[d + r] = values[m + r] = 0u;
    for (unsigned i = 0; i < bytes; i++) {
        bool odd = (i / element_bytes) % 2u != 0u;
        values[d + i / 8u] |= (uint64_t)(odd ? b[i - element_bytes] : a[i]) << ((i % 8u) * 8u);
        values[m + i / 8u] |= (uint64_t)(odd ? b[i] : a[i + element_bytes]) << ((i % 8u) * 8u);
    }
}

static void test_a8_neon_transpose_registers(void) {
    CHECK(a8_neon_transpose(0u, 2u, 0u, 17u, 16u) == 0xf3fa10a0u &&
          a8_neon_transpose(1u, 2u, 0u, 17u, 16u) == 0xfffa10a0u &&
          a8_neon_transpose(0u, 0u, 1u, 30u, 0u) == 0xf3f2e0c0u, "VTRN encoding anchors");
    for (unsigned thumb = 0; thumb < 2u; thumb++)
      ...
      for (unsigned quad = 0; quad < 2u; quad++)
       for (unsigned d = 0; d < 32u; d += quad + 1u)
        for (unsigned m = 0; m < 32u; m += quad + 1u) {
            ...
            if (d != m) a8_transpose_expected(expected, size, quad, d, m);
            uint32_t flags = c.cpsr, fpscr = c.vfp_fpscr;
            CHECK(a8_move_step(&c, thumb, a8_neon_transpose(thumb, size, quad, d, m)) == (d == m ? ARM_UNDEFINED : ARM_OK) &&
                  c.r[15] == (d == m ? 0x100u : 0x104u) && c.cycles == 1u &&
                  c.cpsr == flags && c.vfp_fpscr == fpscr && c.excl_valid && c.excl_addr == 0x2468u && c.a8_excl_size == 8u,
                  "transpose register/status T=%u size=%u Q=%u D=%u M=%u", thumb, size, quad, d, m);
            bool same = true;
            for (unsigned r = 0; r < 32u; r++) same &= vfp_get_d(&c, r) == expected[r];
            for (unsigned r = 0; r < 15u; r++) same &= c.r[r] == 0u;
            CHECK(same, "transpose ordering, bank boundary, other registers or UNKNOWN operand guard");
        }
}

static void test_a8_neon_transpose_access_and_host_state(void) {
    static const unsigned permissions[] = {0u,1u,3u};
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned size = 0; size < 3u; size++)
      for (unsigned quad = 0; quad < 2u; quad++)
        ...
        for (unsigned enabled = 0; enabled < 2u; enabled++)
         for (unsigned access = 0; access < 3u; access++)
          for (unsigned same = 0; same < 2u; same++) {
            ...
            bool allowed = enabled && (permissions[access] == 3u || (permissions[access] == 1u && !user));
            if (allowed && !same) a8_transpose_expected(expected, size, quad, 30u, 16u);
            uint32_t flags = c.cpsr, fpscr = c.vfp_fpscr;
            CHECK(a8_move_step(&c, thumb, a8_neon_transpose(thumb, size, quad, 30u, same ? 30u : 16u)) ==
                  (allowed && same ? ARM_UNDEFINED : ARM_OK) && c.vfp_fpscr == fpscr, "transpose access disposition");
            CHECK(allowed ? c.r[15] == (same ? 0x100u : 0x104u) && c.cpsr == flags :
                  ...
                  "transpose access must precede the UNKNOWN-result guard");
            for (unsigned r = 0; r < 32u; r++) CHECK(vfp_get_d(&c, r) == expected[r], "transpose denied/UNKNOWN register mutation");
          }
    fenv_t saved; CHECK(fegetenv(&saved) == 0, "save transpose host FP state");
    static const int rounds[] = {FE_TONEAREST,FE_UPWARD,FE_DOWNWARD,FE_TOWARDZERO};
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned size = 0; size < 3u; size++)
      for (unsigned quad = 0; quad < 2u; quad++)
       for (unsigned host = 0; host < 4u; host++) {
        ...
        expected[16] = UINT64_C(0xff81234580000001); expected[17] = UINT64_C(0x7fcabcde807fffff);
        expected[30] = UINT64_C(0x0123456789abcdef); expected[31] = UINT64_C(0xfedcba9876543210);
        for (unsigned r = 0; r < 32u; r++) vfp_set_d(&c, r, expected[r]);
        a8_transpose_expected(expected, size, quad, 30u, 16u);
        CHECK(fesetround(rounds[host]) == 0 && feclearexcept(FE_ALL_EXCEPT) == 0 && feraiseexcept(FE_DIVBYZERO) == 0,
              "prepare transpose host FP state");
        int exceptions = fetestexcept(FE_ALL_EXCEPT);
        CHECK(a8_move_step(&c, thumb, a8_neon_transpose(thumb, size, quad, 30u, 16u)) == ARM_OK && c.vfp_fpscr == 0xfff79f9fu &&
              fegetround() == rounds[host] && fetestexcept(FE_ALL_EXCEPT) == exceptions, "transpose touched FP flags/host state");
        for (unsigned r = 0; r < 32u; r++) CHECK(vfp_get_d(&c, r) == expected[r], "transpose host-dependent result");
       }
    CHECK(fesetenv(&saved) == 0, "restore transpose host FP state");
}

static void test_a8_neon_transpose_invalid_and_it(void) {
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned enabled = 0; enabled < 2u; enabled++)
      for (unsigned skip = 0; skip < (thumb ? 2u : 1u); skip++)
       for (unsigned bad = 0; bad < 5u; bad++) {
        arm_cpu_t c; a8_move_reset(&c, thumb);
        c.vfp_fpexc = enabled ? ARM_FPEXC_EN : 0u; if (skip) c.cp15.cpacr = 0u;
        uint32_t insn = a8_neon_transpose(thumb, bad == 0u ? 3u : 2u, 1u,
            bad == 1u ? 31u : 30u, bad == 2u ? 17u : bad == 3u ? 30u : 16u);
        if (thumb) { m_w16(NULL, 0x100u, skip ? 0xbf08u : 0xbf18u); CHECK(arm_step(&c) == ARM_OK, "transpose IT setup"); }
        uint32_t pc = c.r[15], flags = c.cpsr, fpscr = c.vfp_fpscr;
        bool denied = !enabled && bad >= 3u, valid = bad == 4u;
        CHECK(a8_move_step(&c, thumb, insn) == (skip || valid || denied ? ARM_OK : ARM_UNDEFINED), "transpose invalid/IT disposition");
        if (skip || (!valid && !denied)) CHECK(c.r[15] == (skip ? pc + 4u : pc) &&
            c.cpsr == (skip ? flags & ~0x0600fc00u : flags), "transpose invalid/UNKNOWN/skipped state");
        else if (denied) CHECK(c.r[15] == ARM_VEC_UNDEFINED && c.spsr[ARM_BANK_UND] == flags, "transpose disabled valid instruction");
        else CHECK(c.r[15] == pc + 4u && c.cpsr == (flags & ~0x0600fc00u), "transpose valid IT retirement");
        CHECK(c.vfp_fpscr == fpscr, "transpose invalid/IT FP status");
        for (unsigned r = 0; r < 32u; r++) CHECK(vfp_get_d(&c, r) == 0u, "transpose invalid/IT register mutation");
       }
    static const unsigned fields[] = {4u,7u,8u,11u,16u,17u};
    for (unsigned thumb = 0; thumb < 2u; thumb++)
     for (unsigned enabled = 0; enabled < 2u; enabled++)
    ...
    const arm_arch_t legacy[] = {ARM_ARCH_V6_ARM1176,ARM_ARCH_V7_SWIFT};
    for (unsigned profile = 0; profile < 2u; profile++) {
        arm_cpu_t c; CHECK(arm_reset_profile(&c, &g_bus, legacy[profile]), "legacy transpose reset");
        c.cp15.cpacr = 0x00f00000u; c.vfp_fpexc = ARM_FPEXC_EN;
...
int main(void) {
    ...
    test_a8_vfp_binary_vectors();
    test_a8_vfp_binary_access_and_invalid();
