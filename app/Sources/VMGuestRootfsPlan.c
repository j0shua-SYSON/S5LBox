...
static bool plan_script_apt_cache_tool(
    ...
    return plan_script_append(plan, "cache_archive=\"$packages/") &&
           ...
           plan_script_append(
               ...
               "    fi\n"
               "    if generate_caches && publish_caches; then\n"
               "        cache_ready=1\n"
               "    fi\n"
               "fi\n"
               "if [ \"$cache_ready\" -eq 1 ]; then\n"
               "    echo 'Cydia APT caches recovered outside the application watchdog'\n"
               "else\n"
               "    echo 'Cydia cache recovery deferred; existing caches were left untouched'\n"
               "fi\n"
               "/bin/sync\n"
...
static bool plan_compute_manifest(vm_guest_rootfs_plan_t *plan,
    ...
    if (!ios3_sha256_init(&context) ||
        !plan_hash_text(&context, "s5lbox-guest-rootfs-plan 9\n") ||
